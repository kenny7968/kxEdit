...
public sealed class FileController
{
    ...
    private bool WriteToPath(Document doc, string path)
    {
            ...
            )
        {
            ...
            // L=190 なら tmp パスは 1 文字も残らない(網の fixture は L=220)。
            //
            ...
            //     ユーザーが他所から知る手段が無い。この非対称は「tmp を切らない」だけでなく
            //     <b>「原本はもっと切ってよい」</b>も導くので、原本側の上限は共通文言の 200 ではなく
            //     80 にする(レビュー指摘 2・SR の読み上げが約 110〜130 文字短くなる)。
            //     <b>丸めても操作に必要な情報は落ちない</b>: 退避先のフォルダーは tmp パス側に常に
            //     完全な形で載るので、丸められるのは「どこへ保存しようとしていたか」という文脈情報
            //     だけである(通常の保存経路のように原本が正規化済み絶対パスなら、tmp パスは原本パスに
            //     乱数と .tmp を足したものなので、原本の完全な形も同じ文中に残る)。
            //     逆向きの案「tmp をファイル名だけにして『同じフォルダー』と言う」は採らない ——
            //     原本を丸めた場合にフォルダーが判らなくなる。
            ...
            if (ex is AtomicReplaceFailedException replaceFailed)
            {
                ...
                //
                // 語順は SR を前提に決めている(レビュー指摘 3): SR は線形に読むので、退避先の長い
                // パスを先に置くと、ユーザーは数百文字の朗読を聞き終えるまで「次に何をすればよいか」に
                // たどり着けない。しかも<b>最短の復旧は退避先を探すことではない</b> —— 失われたのは
                // 元ファイルの方で、退避先 tmp はエディタが今も持っている内容の複製でしかないから、
                // 「名前を付けて保存」で別の場所へ出すのが最短である。したがって<b>復旧手段を長い
                // パスより前に置く</b>。tmp の案内はその後ろで「にも残してあります」と足す。
                string message = System.IO.File.Exists(replaceFailed.PreservedTempPath)
                    ? $"保存できませんでした: 保存先 '{target}' が失われました。"
                        + "編集中の内容はまだ開いたままなので、「名前を付けて保存」で別の場所へ保存できます。"
                        + $"書き込んだ内容は '{SanitizeForDisplay.OneLine(replaceFailed.PreservedTempPath)}' にも残してあります。"
