...
public static class SessionLayoutStore
{
    ...
    /// <summary>%APPDATA%\kxEdit\session-state.json(SettingsStore.DefaultPath と同ディレクトリ)。</summary>
    ...
    public static SessionLayout? Load(string path)
    {
        try
        {
            ...
            if (info.Length > MaxLoadFileSizeBytes)
            {
                System.Diagnostics.Trace.TraceWarning(
                    "kxEdit: session-state.json too large ({0} bytes); ignoring.",
                    info.Length
                );
                return null;
            }
        ...
        catch (Exception ex)
        {
            ...
            System.Diagnostics.Trace.TraceWarning(
                "kxEdit: session-state.json unreadable ({0}); ignoring.",
                ex.GetType().Name
            );
    ...
    internal static SessionLayout Normalize(SessionLayout raw)
    {
        ...
        foreach (var t in source)
        {
            ...
            {
                System.Diagnostics.Trace.TraceWarning(
                    "kxEdit: session-layout-tabs-capped ({0} -> {1})",
                    source.Count,
                    MaxTabs
                );
                break;
            ...
            if (backupId is not null && !BackupIdValidator.IsValid(backupId))
            {
                ...
                System.Diagnostics.Trace.TraceWarning(
                    "kxEdit: session-layout-invalid-backup-id ({0})",
