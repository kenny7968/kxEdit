...
public static class LastSessionBuffersStore
{
    ...
    /// <summary>%APPDATA%\kxEdit\last-session-buffers.json(SettingsStore.DefaultPath と同ディレクトリ)。</summary>
        ...
        try
        {
            ...
            if (info.Length > MaxLoadFileSizeBytes)
            {
                System.Diagnostics.Trace.TraceWarning(
                    "kxEdit: last-session-buffers.json too large ({0} bytes); ignoring.",
                    info.Length
                );
                return new Dictionary<string, string>();
            }
