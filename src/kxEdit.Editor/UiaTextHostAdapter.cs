...
//   - SetSelectionCharRange / ScrollCharRangeIntoView / Focus (UI スレッド専用の caret / 選択 /
//     スクロール操作) = host 経由
//
...
internal class UiaTextHostAdapter : IUiaTextHost
{
    ...
    int IUiaTextHost.WordStart(int offset)
    {
        ...
        if (snap is null)
            return 0;
        int o = Math.Clamp(offset, 0, snap.CharLength);
        return kxEdit.Core.Editing.WordBoundary.WordStart(
            snap,
            o,
            kxEdit.Core.Editing.WordBoundary.DefaultMaxScan
        );
    }
    ...
    int IUiaTextHost.WordEnd(int offset)
    {
        ...
        if (snap is null)
            return 0;
        int o = Math.Clamp(offset, 0, snap.CharLength);
        return kxEdit.Core.Editing.WordBoundary.WordEnd(
            snap,
            o,
            kxEdit.Core.Editing.WordBoundary.DefaultMaxScan
        );
    ...
    int IUiaTextHost.NextWordStart(int offset)
    {
        ...
        if (snap is null)
            return 0;
        int o = Math.Clamp(offset, 0, snap.CharLength);
        return kxEdit.Core.Editing.WordBoundary.NextWordStart(
            snap,
            o,
            kxEdit.Core.Editing.WordBoundary.DefaultMaxScan
        );
    ...
    int IUiaTextHost.PrevWordStart(int offset)
    {
        ...
        if (snap is null)
            return 0;
        int o = Math.Clamp(offset, 0, snap.CharLength);
        return kxEdit.Core.Editing.WordBoundary.PrevWordStart(
            snap,
            o,
            kxEdit.Core.Editing.WordBoundary.DefaultMaxScan
        );
    ...
    // === スクロール (interface の節順に合わせてここに置く: 座標 → スクロール → 属性) ===
