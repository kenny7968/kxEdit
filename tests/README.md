...
| 層 | プロジェクト | 責務 | 実行 |
|---|---|---|---|
| L1 単体 | `tests/kxEdit.Core.Tests` | 純ロジック(Buffer/Layout/Search/Csv/Text/Settings/Backup/Editing/IO)+ kxEdit.Accessibility の UIA プロバイダ契約 | 毎マージ(自動) |
| L2 コンポーネント | `tests/kxEdit.Editor.Tests` | EditorControl 実生成(STA)での編集・キャレット・選択・UIA イベント・IME・クリップボード | 毎マージ(自動) |
| L3 App 統合 | `tests/kxEdit.App.Tests` | Controller 群・DocumentManager・Speech 経路選択のロジック(Form 境界・OS 境界を Fake 化) | 毎マージ(自動) |
| L4 性能ゲート | `tests/kxEdit.Core.Bench` / `tests/kxEdit.Editor.Smoke --bench --bench-save` | 1GB 級文書・タイピング応答 5µs 等の EXIT コード判定 | 手動(Buffer/Layout/IO 大変更時) |
...
5. **Accessibility の契約テスト**: kxEdit.Accessibility 専用テストプロジェクトは作らない=プロバイダ契約は Core.Tests・UIA 統合は Editor.Tests に置く。
...
L3 の新テストを書くときはまず `tests/kxEdit.App.Tests/GrepControllerTests.cs` を読み、以下の三本柱を写す:
...
2. **`HostForm.CreateWithDocs()` 呼び出し**(`tests/kxEdit.App.Tests/TestHost.cs`)= 画面外・非アクティブ・タスクバー非表示で「可視状態」まで作る共通ヘルパ。TabControl の Selected/Deselecting 系は可視で同期発火するため、これを Host 内から必ず経由する。
...
- `tests/kxEdit.App.Tests/Fakes/FakeAnnouncer.cs` = `List<string> Said`
- `tests/kxEdit.App.Tests/Fakes/FakePrompt.cs` = `List<(string Kind, string Text, string Caption)> Log` + `OkCancelResult` 事前登録
- `tests/kxEdit.App.Tests/Fakes/FakeFileDialogService.cs` / `FakeBackupWriter.cs` / `FakeRestorePrompt.cs` / `FakeTimeProvider.cs` / `FakeCellPicker.cs` / `FakeGrepView.cs` / `FakeGrepResultsView.cs` / `FakeFindReplaceView.cs` / `FakeGrepSearchFn.cs` = すべて同じ「戻り値/応答は事前登録・呼ばれた引数を List/Queue で順序保持」
...
- **Editor.Tests**: `tests/kxEdit.Editor.Tests/Sta.cs`(同型)
...
6. **文言 assert は Core 定数参照か**: 例 `CsvAnnounceFormatter.Cell(...)` / `CsvAnnounceFormatter.LeftEdge` / `CsvAnnounceFormatter.ModeOn` 等の `src/kxEdit.Core/Csv/CsvAnnounceFormatter.cs` に定義済み定数を参照する。**リテラル手書き禁止**=文言の正は Core に一元化。
...
- `tests/kxEdit.Editor.Tests/ClipboardTests.cs`(クラスレベル・2026-07-15 ブランチ 4 で付与)
...
- **`.github/workflows/ci.yml` / `release.yml`(CI)**: `dotnet test kxEdit.sln -c Release --no-build --filter "Category!=LocalOnly"` で隔離。
