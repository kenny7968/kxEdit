...
public class BackupStoreTests
{
    ...
    // の白リスト検証を行い、%APPDATA%\kxEdit\backups 配下に攻撃者が植えた不正 Id JSON を復元候補から
    ...
    // -------------------------------------------------------------------------
    // 現状のフラット配置(%APPDATA%\kxEdit\backups\ 直下)は、複数 kxEdit インスタンス同時起動時に
    // 別インスタンスが「すべて破棄」を選ぶと自インスタンスのライブ backup が消える問題があった。
    // BK-M-2 は書込先を %APPDATA%\kxEdit\backups\session-{Guid.N}\ へ移し、
