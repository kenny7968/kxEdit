...
**Step 0: `FindReplaceDialog` の 3 対 3 の切り分けに網を張る(Task 6 レビュー S-1)**

**本テーマの成否は G-2 の 3 箇所が無傷であることに全面的に依存する**が、現状
`FindReplaceDialog` を直接触るテストが 1 本も無い(App.Tests で参照するのは
`Fakes/FakeFindReplaceView.cs` だけ)。つまり **`_next.Click` の `Hide()` を
`HideByUser()` に変異させても App.Tests 444 件は全緑**である。
将来「6 箇所を統一しよう」と揃えられても、キャッシュが F3 のたびに落ちるだけで
テストは何も言わない。

Task 6 単体では挙動不変なのでブロッカーではなかったが、**本タスクで購読が入った瞬間から
本番の挙動差になる**ので、ここで張る。

実 Form + リフレクションで private フィールドを叩く既存パターンが
`tests/yEdit.App.Tests/MainFormSmokeTests.cs:899` と
`tests/yEdit.App.Tests/GrepControllerTests.cs:471` にある。これに倣って:

| 経路 | 期待する `Dismissed` 発火回数 |
|---|---|
| 閉じるボタン / Escape / `OnFormClosing(UserClosing)` | **1** |
| `_next` / `_prev` / Enter(G-2 の一時退避) | **0** |

**張ったら必ず、G-2 の 1 箇所を `HideByUser()` へ変異させてテストが赤くなることを確認すること**
(設計書 §7.6 — 網は書いただけでは網ではない)。
...
> **購読の張り方(Task 6 実装者の申し送り)**
>
> `Open` は `_view is null || _view.IsDisposed` のときだけ view を再生成する。
> **購読は必ずその `if` ブロックの中に置くこと**(生成のたびに 1 回)。
> ブロックの外に置くと `Open` を呼ぶたびに購読が積み上がり、`DropSearcher` が多重に走る。
>
> 古い view の購読解除は不要。破棄済み view はイベントごと落ちるため、
> 購読が古い view を生かし続けることはない(参照の向きは view → controller)。
>
> **`DropSearcher` は冪等でなければならない。** `Dismissed` は連続発火しうる
> (Escape → 再表示 → また Escape)。フィールドを null にするだけなら問題ない。

> **テストでの使い分け(Task 6 実装者の申し送り)**
>
> `FakeFindReplaceView` には 2 つの経路がある。**取り違えると設計意図と逆のことを検証してしまう**:
>
> | やりたいこと | 使うもの |
> |---|---|
> | ユーザーが検索を終えた(破棄される) | `RaiseDismissed()`(`Visible = false` も同時に立つ) |
> | **G-2 の一時退避**(非表示でも searcher は生きている) | `Visible = false` を**直接代入** |
>
> 後者で「非表示のまま F3 を連打してもキャッシュが生きている」ことを書けば、
> 本設計の核(`!Visible` を破棄トリガに使わない)がそのままテストとして表現される。
