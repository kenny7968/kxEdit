...
- **L5（実機 SR 検証）は必要**と判断する。読み上げ経路自体は不変だが、CSV モードは
  ...
  - **（レビュー指摘 I-3 で追加）** 「CSVモード終了」の発声が、**押した本人が確実に気づける**
    音量・タイミングで届くこと。終了直後は `ReadOnly=false` に戻るため、モードが外れたことに
    気づかないままグリッド用の素キー（`C` / `R` / `G` / 矢印）を押すと本文への文字入力・
    キャレット移動になる。Esc は反射的に押されやすく、モードメニュー再トグルより誤発火
    しやすいぶん、この発声が唯一の気づきの手掛かりになる。
...
- `説明書/kxEdit説明書.md` 8.1 のキー表への追記が必要。**ユーザー編集版が正**
  ...
  ```
  | Esc | CSVモードを終了する(セル編集中は編集の取消) |
  ```

  括弧書きは**レビュー指摘 I-2 による精密化**。同書 8.2「セルの編集(F2)」に既に
  「**Esc**: 編集を取り消します。」があり、8.1 へ無注釈で `Esc` を足すと同じキーに
  2 つの意味が並んで読めるため。

- **説明書の未反映が 2 件たまっている**。本件の `Esc` に加え、PR #78 の `Ctrl+矢印`
  （行/列の端へ移動）も 8.1 のキー表に未反映のまま。ユーザー校閲の際は 2 件まとめて
  検討してもらう。
