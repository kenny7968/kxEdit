...
### 最終ブランチレビューの反映後の再実行(同日)

最終レビュー(コード品質パス・脆弱性パス)の反映で、harness に次を入れた。
- 入力の効果の自己チェック: フォーカス・変更済みの表示・本文長・検索欄の文字列・未保存のタブ数・Ctrl+Tab の一巡・矩形数。
- `flags` 列(`quiet_timeout`)と `env` 行(版・exe の SHA256・画面・シナリオ・status)。
- 目印の `cleared` 状態、Job オブジェクト、入力直前の pid 照合(C# 側)、クリップボードは戻さずに空にする。

そのうえで全シナリオを再実行し、すべて自己チェックを通った(NVDA 起動中)。

**1 回ごとのばらつき**が大きい値がある。
- 「基準: 文書先頭での ←」は ja10k で 5.5 → 15.8 → 5.5 ms と変わった。M-7 の 40 行は 28 → 5.5 ms と変わった。
- どちらも kxEdit の外(IME/TSF・NVDA・初回の UIA 接続)の費用が乗る値である。
- harness の値で採否を判断するときは、設計書 §3.2 のとおり 3 回走らせて最小〜最大を見ること(README §3 に記載)。

再実行で主要な値は上の表と同程度だった。
- 全面の再描画: ja10k 8.4〜8.6 ms・en10k 6.7〜6.9 ms
- →← の交互: ja10k 17.2〜19.1 ms
- M-3 ののこぎり型: 25.0 → 17.2 ms
- M-4: 15.4 / 15.6 ms
- M-5: 11.4 / 13.1 / 12.0 ms
- M-6: 99.6 / 107.0 / 112.5 ms。UI スレッドは 61.7〜68.0 ms、その他で最大のスレッドは 28.5〜32.4 ms。
- M-7 の全文: 2,812 ms

### レビュー指摘の採否(最終ブランチレビュー)

- コード品質パス: I-1〜I-4 と M-1〜M-11 をすべて採用した。I-4(投函・タイマーに回した処理を Smoke は計上しない)は、PerfBench のクラス doc と README に明記する形で対応した。
- 脆弱性パス: I-1〜I-3 と M-1〜M-7 を採用し、M-8 は却下した。
  - M-8 は PerfBench の `--json` の上書きの指摘。開発者が渡す引数で、利用者データに触れないため却下した。
  - I-3 の「-Recover で退かせる側が新しければ -Force を求める」は採らなかった。計測の産物でも常に新しくなるので判別に使えないため。代わりに退避の日時を表示し、Job オブジェクトで取り残しそのものを防いだ。

### 申し送り

- Smoke `--perf` は計測中にメッセージを汲まない。処理を BeginInvoke・タイマー・間引きに回すフェーズ(3・5 など)は、Smoke の値だけで効果を判断せず、perf-harness でも確かめること。
- 本フェーズの現状値は NVDA 起動中のもの。NVDA なしの値が要るフェーズ(とくに 5・6)は、変更前の値も NVDA なしで採り直すこと。
