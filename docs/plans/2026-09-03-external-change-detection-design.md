...
- **到達不能記憶の効き方(逸脱 11 の後)**: 記憶を見るのは復帰・タブ切替の検知と A-1 の起動時復元だけ。
  ...
  「到達可能だが遅い共有で保存直前のプローブが 5 秒でタイムアウトした保存」で、その保存は確認なしで
  上書きする(遅い共有なら繰り返しうる。「取得できなければ判定しない」契約どおり)(A-1 の復元では null = 従来どおり復元で安全側だが、M-18 では安全側ではない)。同じ類の極小窓として、
  書いた後のプローブが(書込直後に共有が落ちて)タイムアウトすると基準が null になる。次の保存で回復する。
...
- **脆弱性パス**: Critical / High / Medium ゼロ。Low 2(V-1 = 逸脱 11 / V-2 = `Remove` コメントの範囲。ともに解消)/
  Info 4(受容。§11.4)。
