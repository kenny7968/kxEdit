...
- 変更前の計測(Task 2)を、Task 4・5 の実装の後に回した。計測する publish(`pub-before`)は、src が main と同一の時点(`f21fcf7`)で先に作っておいた。値は変わらず、ユーザーが PC を明け渡す時間を 1 回にまとめられるため。
- 計測(2026-09-26)は、ETW 2 本 → M-6 を 12 回(変更前 → 変更後の順で OFF・ON を交互に 3 巡)の順で行った。すべて EXIT 0・`status=completed`・NVDA 起動中。

### Task 1: harness の `-SessionRestore`

- SelfTest に 5 件足した(空にする前は書かない・OFF では書かない・空にした後は書く・目印の元の場所と違えば書かない、ほか)。38 件 PASS。
- 前倒しの脆弱性レビュー: Critical なし。
  - Important 1 件(M-1 が `-SessionRestore` を反映しないのに、`env` 行は 1 になる)を fixup で反映した。
  - Minor 2 件(目印の元の場所の照合・検査が投げた理由の照合)も fixup で反映した。
  - Minor 1 件(目印が null のとき・フォルダーがないときの分岐の SelfTest)は見送った。どちらも 1 行の分岐で、誤っていても harness は安全側に中止する。

### Task 3: ETW の調査

- 結果は設計書 §11.1.1 に書いた。主因は UIA(サーバーの I/O スレッド・UI スレッドに回ってくる呼び出し・NVDA の注入 DLL)で、kxEdit では減らせない。
- §11.3 は、ユーザーの判断で 2 項目とも採らない(2026-09-26)。そのため Task 5 Step 7 は「採用なし」とする。

### Task 4・5

- Task 4(P-6): 新しいテスト 9 件。陰性対照を 3 つ試し、どれもテストが落ちることを確かめた。
  - 省略の条件を外す → 3 件 FAIL
  - Write 分岐で参照を覚えるのをやめる → 1 件 FAIL
  - ForceWrite の条件を外す → 6 件 FAIL(既存テストを含む)
  - 陰性対照のビルドは警告=エラーで止まる(未使用の private)。`-p:TreatWarningsAsErrors=false` で回した。
- Task 5: seam を先に足し、現状の挙動でテストが落ちることを確かめた(期待 1・実際 2)。陰性対照を兼ねる。
- Task 5 Step 7: 採用なし(上記)。

### 計測(harness M-6・CPU ms/回・3 回の 最小 / 中央値 / 最大・NVDA 起動中)

プロセス全体:

| セッション復元 | 条件 | 変更前 | 変更後 |
|---|---|---|---|
| OFF | 空の新規タブ 4 枚 | 88.3 / 94.5 / 95.3 | 94.9 / 94.9 / 99.6 |
| OFF | 3 ファイル+空 | 95.7 / 100.4 / 103.1 | 104.3 / 104.7 / 108.6 |
| OFF | 3 ファイル未保存+空 | 98.8 / 102.0 / 104.3 | 100.8 / 103.1 / 103.1 |
| ON | 空の新規タブ 4 枚 | 93.8 / 95.7 / 96.1 | 91.4 / 93.8 / 95.3 |
| ON | 3 ファイル+空 | 103.5 / 107.0 / 109.4 | 102.0 / 111.3 / 111.7 |
| ON | 3 ファイル未保存+空 | 108.6 / **110.2** / 111.3 | 104.7 / **105.5** / 107.8 |

- 揺れの範囲を超えて改善したのは、ON の「3 ファイル未保存+空」(110.2 → 105.5 ms)だけ。変更後の最大(107.8)が変更前の最小(108.6)を下回った。ETW で見た P-6 の費用(3.7 ms/回)と同程度である。
- OFF の「3 ファイル未保存+空」は揺れの範囲に収まった(102.0 → 103.1)。
- 他の条件は、P-6 の対象外(未保存タブがない)。変わらないのが期待どおりで、ほぼ揺れの範囲だった。
- **例外: OFF の「3 ファイル+空」は、変更後の最小(104.3)が変更前の最大(103.1)を 1.2 ms 上回った**(中央値で +4.3 ms)。
  - この条件で変わるコードは、クリーンな文書のスナップショット参照の取得(`CurrentBuffer.Current`・ナノ秒級)だけである。全文化も投函も変更前と同じだけ起きる。
  - ON の同じ条件では、範囲が重なった(107.0 → 111.3、変更前の最大 109.4・変更後の最小 102.0)。
  - 以上から計測の揺れと判断した。PR に記載し、採否をユーザーに確認する(設計書 §3.2)。
- 外部変更チェックの投函の省略(Task 5)の効果は、測れる大きさではない(「空の新規タブ 4 枚」で揺れの範囲)。
- 未保存の合計は約 4.7MB で、P-6 の費用は調査記録(約 10 ms)より小さかった。フェーズ 5 の P-13 で全文化が約 2.4 倍速くなったためと見られる。
