...
### 8.10 Task 3 — 計画のテストに網の穴があった(片側だけの主張・§8.2 と同型)

ミューテーション検証で `changeEnd` の `&& char.IsLowSurrogate(...)` を落とす変異が**生存**した。
計画が指定した `GetExactChangeRange_CrlfSplit_DoesNotWiden` は
**始端側の CRLF 分割しか見ていなかった**(`"abc\r\ndef"` の `(4,1)` = `s0` が CRLF の内側)。
`changeStart` と `changeEnd` は独立した AND ガードなので、片側だけの網では他方が無防備になる。
終端側(`(3,1)` = CR だけを置換 → `e0` が CRLF の内側)の 1 行を同テストへ足して殺した。

**§8.2(b) の「事後条件テストが iff の半分しか主張していなかった」と同型**である。
本ブランチで**片側だけの主張**が 2 回出た。

**決定的なのは、全数プローブではこの変異を殺せないこと。** 全数プローブの主張は
「返した範囲の**外側**は変わらない」という**上位集合**の保証なので、**範囲を過剰に広げる退行は
違反にならない**(実測: 変異適用時も 900 ケース全通過)。等値を主張する狙い撃ちテストだけが捕まえる。

**Task 4 に直接効く性質である。** `SearchController` の包含検査に使うと、過剰に広い返却値は
「安全側」ではなく**正当な置換を誤って拒否する**方向へ効く。狙い撃ちテストの等値主張を弱めないこと。

### 8.11 Task 3 — doc の根拠がまた実態より強かった(3 タスク連続)

`ExactRangeParts` の remarks に計画が与えた文面「`ReplaceCharRangeExact` と
`GetExactChangeRange` が**同じ答えを出す**ことが後者の存在意義」は**偽**だった。
両者は設計上**意図的に違う答えを返す**(CRLF 分割時、後者は広げた範囲を捨てる)。
共有しているのは「答え」ではなく**範囲計算の素材**である。

字義どおり読むと `GetExactChangeRange` の CRLF 分岐が契約違反に見え、削られると
PR #56 §9.9 の根治が打ち消される。**この設計で最もデリケートな判断が doc に守られていなかった。**

**実装者が指摘の因果をさらに精密化した。** 「削ると `SearchControllerTests` の 2 件が赤くなる」は
**現時点では偽**である。`GetExactChangeRange` に production の呼び出し元がまだ無いため、
端ごとの弁別を落とす変異を当てても赤くなるのは **Editor テスト 1 本だけ**で
App は 714 件 green のままだった(実測)。remarks は
**実測(現在の番人は Editor 1 本)と想定(Task 4 後に App 2 件が加わる)を別段落へ物理的に分離**し、
想定側に「(想定・未実測)」を明記してある。[[rationale-not-just-conclusion]] の実践。

**Task 4 完了時に、この最終段落を実測へ昇格させること**(同時に
`GetExactChangeRange` の remarks が用途を `SearchController` と書いている件も実態と一致する)。

### 8.12 計画の記述が誤っていた形の集計(3 タスク連続)

| Task | 計画の記述 | 実態 |
|------|-----------|------|
| 1 | 「同値テストがあるので片方だけ直すと必ず赤くなる」 | 2 本の網は相補的で、どちらか一方では穴が開く |
| 2 | 「全文置換は旧実装と 1 文字も変わらない」 | 再アンカー単独では真。ゼロ幅後退と合成すると 7.3% が変わる |
| 2 | 「赤にならない fixture は網として無効」 | 欠陥網には正しいが**差分ガードには誤り** |
| 2 | 「substring 化を弁別する唯一の形」 | 偽(`(?<=a)aa` が逆向きに弁別する) |
| 3 | 「両者が同じ答えを出すことが存在意義」 | 意図的に違う答えを返す。共有するのは素材 |

**いずれも実装ではなく、計画が根拠として書いた主張のほうが実態より強かった。**
「計画のコードは正解ではない」([[plan-code-is-not-ground-truth]])は**コードだけでなく
計画が書く根拠・判定則・doc 文面にも及ぶ**。次の計画では、根拠を書くときに
「これは実測か、想定か」を計画の時点で分けて書くこと。

### 8.13 Task 3 の運用メモ

- **`EditorControl.SetSource` は 1 度しか呼べない**(2 回目で `InvalidOperationException`)。
  反復するテストは `SetOrReplaceSource` を使う。計画のコードはそのままでは動かなかった。
- **CSharpier + Sonar `S3973` が再発**(§8.4-1)。4 重入れ子ループの波括弧を畳まれ、
  commit 後のリビルドで検知した。**commit 後リビルドの義務化が実際に機能した 2 例目。**

### 8.14 Task 3 のレビュー 2 本 — 「片側だけの主張」が 3 度目・二重実装の対象がすり替わっていた

**Important 3 / Minor 5 を全件反映**(却下ゼロ)。弁別規則そのものは正しいことを、
コード品質レビューが `TextBoundary` の実装からの導出**と** `IsLowSurrogate` →
`GetChar(x) != '\n'` 差し替え変異の**生存**(=ガード下で 2 式が同値であることの実測証明)の
両方で確認している。指摘は「規則が正しいのに、それを正しく保つ仕掛けが足りない」領域に集中した。

**I-1(3 度目の片側だけの主張)。** 両端の判定を 1 つのフラグに括る変異が
**Editor 514 件すべて green のまま生存**した。狙い撃ちの文書が CRLF とサロゲートを
別々にしか持たず、全数プローブの 8 文書にも**同居する文書が 1 つも無かった**。
`SearchControllerTests` の 2 件も番人にならない(CRLF だけの文書では変異と現行が同値)。
mixed 文書 2 形(`"\r\n😀"` / `"😀\r\n"`)の厳密値テストで塞いだ。

**I-2 — 二重実装の対象がすり替わっていた。** 設計書 §2.1 は「規則の二重実装を作らないことが
本 seam の唯一の要求」と書いた。**範囲計算(`ExactRangeParts`)は正しく括り出されたのに、
弁別規則のほうが `EditorControl` へ二重化した。** `TextBoundary` の class doc は呼び出し元での
手書き判定を禁じ、意図的除外の登録簿を持つが `EditorControl` はそこに無かった。
公開述語 `IsSurrogatePairEndingAt(TextSnapshot, int)`(全域関数)を新設して集約した。
**Task 1 の span 版述語と同じ判断**(宣言を弱めるのではなく、規則を括って宣言を真にする)。

**I-2 の副産物 — 登録簿の漏れをもう 1 件発見した。** `EditorControl.Input.cs:88/95`
(WM_CHAR のサロゲート組み立て・A-20 由来)が登録簿に無かった。`:88` は `IsHighSurrogate` なので
**旧登録簿自身の grep 手順でも本来ヒットするはずの取りこぼし**である。登録簿を 5 ファイル・6 箇所へ
更新し、class doc に「棚卸しは `rg IsHighSurrogate` と `rg IsLowSurrogate` の**両方**を掛ける」を
明記した(`rg IsHighSurrogate` だけでは**後退側の弁別**が漏れる。今回漏らしたのがまさにその形)。

**I-3 — このブランチの目的そのものに反する形が残っていた。** 「虚偽の成功発声を消す」のが目的なのに、
`GetExactChangeCharRange` は空範囲を返す=**どんな包含検査も通す**形を無警告で提供していた
(空範囲 → 検査を通る → `ReplaceCharRangeExact` が no-op → 成功発声)。兄弟 API
`ReplaceCharRangeExact` は同型の罠を `<returns>` で明示しており、**doc の非対称**だった。
契約の形は変えず(番兵値を混ぜない方針・照会 API に例外は重い)、警告段落を足した。

**M-2 — 命名を `GetExactChangeCharRange` へ改めた。** `EditorControl` の char オフセット系
public API は**例外なく** `Char` を含む(`ReplaceCharRange` / `SelectCharRange` /
`GetSelectionCharRange` / `EnsureVisibleCharRange` ほか)。production の呼び出し元がゼロの
うちに直した。**設計書 §2.1 の旧名は策定時スナップショットとして残す**(CLAUDE.md §8)。

**M-3 — 指摘の前提が同じ fixup で消える形だった。** 「終端側 `e > e0` が
`GetChar(CharLength)` の throw を防ぐ唯一の load-bearing ガード」は**旧コードでは正しい**が、
I-2 で述語を全域関数にした結果、範囲外読みの危険自体が消えた。実装者は
①述語が全域なので範囲外読みは起きない ②**旧実装では確かに終端側だけが安全性を担っていた**
(指摘を履歴として保存)③残したガードは安全のためでなく「その端が実際に広がった」ことの明示で、
**論理的に冗長=落とす変異は生存する(実測済み)**、の 3 段で書いた。
**生存を隠さず書くことで、後任が「網の欠落」と誤読するのを防いでいる。**

### 8.15 並行レビューの運用上の罠

仕様レビュー担当が、作業ツリー(`<repo>`)を複製した瞬間に `EditorControl.cs` が**変異適用中の状態**で、
baseline が偽の赤を出した。実装者の変異検証とレビューが同時に走っていたためである。
以降は `git archive <commit>` でエクスポートした**不変ツリー**で測り直している。

- **レビュー担当は作業ツリーを直接複製せず `git archive <commit>` を使うこと。**
- **実装者は変異をリポジトリ本体に長く置かないこと**(素早く戻すか複製側で当てる)。
- [[subagent-side-effect-guardrails]] の再発。

### 8.16 ビルド判定パターンの訂正

[[mutation-harness-exit-code-trap]] が定める `grep -E " error [A-Z]+[0-9]+"` は
**`error xUnit2020` を取りこぼす**(`xUnit` は小文字始まり)。コード品質レビュー担当が
実際に見落としかけた。**`grep -E " error [A-Za-z]+[0-9]+"` へ緩め**、
`失敗:|成功!` の有無で**テスト実行への到達も併確認**すること。

### 8.17 Task 4 — `ReplaceAll` のガードは**入れない**(§2.1 の未確定を決着)

§2.1 が未確定とした「`ReplaceAll` 側の拒否分岐に到達できるか」を、実装者とレビュー担当が
**独立に全数探索して決着させた。結論はガード不採用。**

**計画の仮説は実測で否定された。** 「置換文字列の端にサロゲート半身を置いてスコープ端を
ペア内側にする」経路は存在しない。**本文バッファが孤立サロゲートを保持できない**
(UTF-8 往復で U+FFFD へ潰れる)ためで、半身が単独で残らない。

| 探索 | 規模 | 結果 |
|------|------|------|
| 実装者 | 全数プローブ 3 本(条件: `{a, \r, \n, 😀}` を 3 単位まで組んだ全テキスト × 全選択 × {ReplaceOne, ReplaceAll} × 代表パターン / 置換文字列) | スコープ端がペア内側に入る状態 **0 件** |
| レビュー担当(独立) | 選択由来スコープ 6,908 通り + **2,158,000 演算**(全本文 × 全スコープを人工 seed 含む) | 同 **0 件**。`ReplaceAll` の実書込がスコープを外れた回数 **0 件** |

**代わりに別の到達形が見つかった**: 「スコープが 1 点へ潰れ、その点が CRLF の内側」
(`"\ra\n"` の `[1,2)` を捕捉 → `a` 削除 → 本文 `"\r\n"` / スコープ `[1,1)`)。
この状態では **`ReplaceOne` はスコープ外へ書くが `ReplaceAll` は書かない** ——
Task 2 の `ReplaceInRange` が「後退先が範囲始端より前になるゼロ幅マッチを件数に数えない」ため
`count == 0` →「見つかりません」で書込へ到達しないからである。
**Task 2 の修正が Task 4 の攻撃面を 1 つ閉じていた。**

この到達形は計画外だが L3 テスト `ReplaceOne_InSelection_CollapsedScopeInsideCrlf_DoesNotReplace`
として起こした。「空スコープでは検査に入らない」形の退行を**このテストだけ**が落とす。

**再監査の注意**: 探索プローブは commit していない。ソースコメントには**数値ではなく
探索の条件・範囲**だけを残してある(検証できない数字は根拠として機能しないため)。
再監査は数値の突き合わせではなく**条件からの組み直し**で行うこと。

### 8.18 Task 4 — 「死んだ分岐」と「常に偽になる式」の弁別

包含検査の終端側 `change.End > check.End` を落とす変異が**生存**した。
実装者は「網が無いのではなく構造的に発火不能」と主張し、レビュー担当が独立に裏づけた。

発火の必要十分条件は **`check.End` がサロゲートペアの内側にあること**
(`WithinScope` から `e0 ≤ check.End`、広がるのは `e0 == check.End` かつ `e0` がペア内側のときだけ)。
その状態への到達は上記探索で 0 件。**逆に人工的に seed すると 2,630 回発火する**ので、
「評価されない死んだ式」ではなく**「現在の不変条件下で常に偽になる式」**である。

**両側を残す判断の決め手はレビュー担当の整理だった**: `>` → `>=` の変異が **10 本を赤にした**。
つまりこの式は**スコープ付き `ReplaceOne` のたびに境界で評価されており、境界の選び方には
完全に網が張られている**。真枝だけが到達不能なのであって、`ReplaceAll` へ新設するガード
(分岐そのものが未評価・境界にも網なし)とは性質が違う。**2 つの判断は矛盾していない。**

規範「死んだ分岐を残さない」を当てるときは、**分岐が評価されるか**と
**真枝へ到達するか**を分けて見ること。

### 8.19 Task 4 — 論証が「載っている理由」だけでは閉じていなかった

「スコープ端がペア内側に入らない」根拠として当初コメントに書かれていたのは
「捕捉は境界へスナップし、端の隣の文字はスコープ内の置換では変わらない」だった。
**これだけでは閉じない** —— 不変の隣接文字が孤立 low サロゲートなら、スコープ内の置換が
末尾に high サロゲートを置いた瞬間にペアが成立し、`scope.End` はペアの内側へ落ちる。

本当の不変条件は**バッファが孤立サロゲートを保持できないこと**だった。

**実装者は実測値をコメントに書く代わりに、既存の committed な網 2 本を参照する形にした**
(`ReplaceCharRangeExact_LowSurrogateOnly_HighHalfCollapsesToReplacementChar` /
`Unpaired_high_surrogate_is_normalized_to_replacement_char_by_buffer`)。
**再監査可能で、根拠が壊れたら CI が落ちる。** 実測値の転記より強い。この形を既定にしたい。

### 8.20 Task 4 — Step 9 の「想定」は偽だった

Task 3 で `ExactRangeParts` の remarks に「Task 4 で繋がれば `SearchControllerTests` の 2 件も
拒否側へ倒れて赤くなる(**想定・未実測**)」と書いた段落を、実測して**偽と判明**した。

弁別を落として常に広げた範囲を返す変異を当てても **App は 721 件 green のまま**である。理由は
① production の呼び出し元は `ReplaceOne` の包含検査だけで、当該 2 件が CRLF 分割スコープを踏むのは
`ReplaceAll` 経路(§8.17 の決定によりガードなし)② 2 件の中の `ReplaceOne` のヒットは
両端が論理文字境界に乗るので広げた範囲と素の範囲に差が出ない。

**この分岐の番人は依然 Editor テストだけ**である。remarks を実測へ書き換えた。

**想定を「想定」と明記しておいたから、偽と分かった時点で正しく訂正できた。**
§8.12 の「根拠を書くときに実測か想定かを分けて書く」の効果が出た最初の例である。

### 8.21 次回に効く罠 — `Copy-Item` での変異 revert が古い DLL を叩く(§8.4-3 の実害例)

実装者が Task 4 で実際に踏み、**20 分の偽の failure** を追った。

`Copy-Item` はコピー元の `LastWriteTime` を保持するため、**変異前バックアップから復元すると
`.cs` のタイムスタンプが DLL より古くなり、MSBuild が増分ビルドで再コンパイルをスキップする**。
結果、**変異が入ったままの DLL** をテストすることになる。

**検出が難しい**: exit code は正常・警告も出ず・`git status` も clean。
`grep -E " error [A-Za-z]+[0-9]+"` でも捕まらない。気づけたのは、無関係な `internal` メンバを
足したら**同じテストが突然通った**からである。

**対策: 変異の適用も revert も Edit ツールで行う**(新しいタイムスタンプが付く)。
または復元後に `LastWriteTime` を更新する。

実装者は**この発見が過去の測定を無効化していないかを自分で再監査**し、
Step 7 / Step 9 の変異はすべて Edit ツール経由(= 健全)であったことを確認している。
**発見したときに、それが遡って何を無効化するかを確かめる**のは正しい手順である。
