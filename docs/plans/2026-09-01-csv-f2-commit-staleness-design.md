...
### 8.36 最終レビュー Important-3 — 「`ConvertEols` にガードが無いから差し替わる」は理由節が偽(8 回目)

本ブランチが繰り返し書いてきた

> `ConvertEols` は `ReadOnly` を見ない(冒頭ガードが `if (_buffer is null) return false;` だけ)ので、
> CSV モード(`ReadOnly=true`)のままでも本文が差し替わる = これが到達経路の実体そのもの

は、**前半(ガードが無い)は真、後半(だから production で差し替わる)が偽**である。実物を開いて
確認した: `FileController.WriteToPath` は `ConvertEols` の前後で **ReadOnly を明示的に落として戻す**。

```csharp
// src/kxEdit.App/FileController.cs:863-874(main 以前から存在)
bool wasReadOnly = doc.Editor.ReadOnly;
if (wasReadOnly) doc.Editor.ReadOnly = false;
try { eolConverted = doc.Editor.ConvertEols(doc.Editor.EolMode); }
finally { if (wasReadOnly) doc.Editor.ReadOnly = true; }
```

**実害**: 将来 `ConvertEols` に `ReadOnly` ガードを足して「M-25 の到達経路は塞いだ」と判断しても、
**保存経路が先に外すので塞がらない**。逆にその改変を入れると T1 / T2 が(production 挙動は不変なのに)
落ち、失敗メッセージが誤った場所を指す。

**同じ理由節が 6 箇所に伝播していた**(行番号はすべて本節執筆時に自分で開いて確認した):

| # | 箇所 | 状態 |
|---|------|------|
| 1 | `tests/kxEdit.App.Tests/CsvControllerTests.cs:742-743` | **本タスクで書き直した**。あわせて T1 / T2 / 強調復元テストの `ConvertEols` 呼び出しを `MutateBodyWhileEditing`(= production と同じ ReadOnly 昇降)経由へ寄せた |
| 2 | 本書 `:450`(§8.12 冒頭) | 「`ReadOnly=true` のまま `true` を返し本文を差し替えた」—— **テストの観測としては真**。ただし production の機構の説明としては不十分 |
| 3 | 本書 `:457-459`(§8.12 末尾) | 「冒頭ガードは `_buffer is null` **だけ**」…「Ctrl+S だけが到達するとした理由が、コードの非対称として実物で確かめられた」—— **帰属が偽**。非対称の実体は「保存経路だけが意図的に ReadOnly を落とすこと」 |
| 4 | `docs/plans/2026-09-01-csv-f2-commit-staleness-l5-checklist.md:30` | 「`ConvertEols` が `ReadOnly=true` のまま本文を差し替えること」(docs 担当が対応) |
| 5 | 実装計画 `2026-09-01-csv-f2-commit-staleness.md:161`(Task 2 Step 1 見出し)/ `:169` / `:172` | **誤りの発生源**。策定時スナップショットなので書き換えず、ここで訂正する |
| 6 | 実装計画 同 `:628`(Task 4 Step 2) | 「`ConvertEols` が ReadOnly=true でも本文を差し替えたか(§4.4 の論証の実測化)」。同上 |

**正しい記述**: 「production の Ctrl+S 経路は `FileController.WriteToPath:863-874` が `ConvertEols` の
前後で ReadOnly を落として戻す。`ConvertEols` にガードを足してもこの経路は塞がらない。」

**レビューの行番号との差**: 指摘は `FileController.cs:862-874` としていたが、`:862` は `ApplyEol(doc);` で
ReadOnly 昇降の idiom は **`:863-874`**。1 行分の差を訂正する(他の 5 箇所は指摘どおり)。

### 8.37 最終レビュー Important-2 — 「`f` が読まれるのは 3 か所だけ」が 4 か所(§8.15 の再発)

`CsvController.BeginEdit` 冒頭の列挙が 3 か所のままだったが、**Task 3 が同じメソッドの 17 行下
(`:268`)に 4 か所目**(`string startValue = CsvWriter.NormalizeEols(f.Value);`)を足していた。

落ちていた 4 番目は、**唯一「`f` 由来の値がクロージャの向こう側へ渡る」箇所**である
(渡るのは正規化済みの**値のコピー**だけで `Start` / `Length` は渡らない)。「`f` から確定時へは
何も渡らない」と読んだ監査者を誤らせる。後半の「いずれも `_editor.Begin` が戻るまでに
同期的に読み切られる」は 4 番目についても真なので、**結論は正しく列挙だけが偽**。

**§8.15 が一度直した箇所(当時「2 か所」→ 3 か所)の再発**である。4 か所へ直し、
4 番目に「渡るのは値のコピーだけ」を添えた。

### 8.38 最終レビュー Important-1 — 開始側の `NormalizeEols` に網が 1 本も無かった

**最も重い指摘。** `startValue = CsvWriter.NormalizeEols(f.Value)` の正規化は**全 fixture で恒等**
だった。F2 **開始時点**でセル値に CR を含む fixture が 1 つも無かったからである
(`MixedEolCsv` の `x\ny` / `ShiftOnlyCsv` の `p\nq` はどちらも LF のみ、他は改行なし)。
したがって開始側だけを落とす変異 `string startValue = f.Value;` は**既存の網すべてを素通りして
生存していた**(実測: 鏡像テストを足した後の App 734 本で落ちるのは**その 1 本だけ** = 残り 733 本は
全緑。`CsvController` は `kxEdit.App` の型なので Core.Tests は原理的にこの変異へ触れない)。

§8.24 の別枠 m5 は「捕捉側と比較側を**両方**落とす」変異で、それは T1 が殺す。しかし読者は
「EOL 正規化には T1 という網がある」と受け取るので、**片側が無防備であることが見えなかった。**

**そしてこれは到達不能枝の話ではない。** 引用符内が CRLF・行区切りが LF の混在 CSV を F2 編集中に
Ctrl+S すると `ConvertEols(Lf)` がセル内 CRLF を LF へ書き換える。開始側を正規化しないと
`"x\r\ny"` と `"x\ny"` が不一致になり、**正常な編集が拒否されて `CommitTargetChanged` が読まれ、
ユーザーの入力が捨てられる**。既存 fixture は変換の向きが逆(セル内 LF → CRLF)なので、
この向きを 1 本も踏んでいなかった。

**対応**: 鏡像 fixture `CrlfInCellLfBodyCsv = "a1,a2\n\"x\r\ny\",b2\nc1,c2"` と
`ConvertEols(LineEnding.Lf)` のテストを 1 本追加(`Commit_AfterEolConversionToLf_...`)。
副次的に **CSV 経路で一度も踏んでいなかった `ConvertEols(Lf)` の向き**も踏むようになった。

開始側だけを落とす変異を注入して観測した赤(逐語):
...
= 確定が拒否されてセルが編集前(LF 化された `"x\ny"`)のまま残る。同じ実行で T1 / T2 は緑のままで、
**「T1 では開始側を守れない」というレビューの主張も同時に実証された**。

**片側ずつの実測(完全な鏡像)**:

| 変異 | 落ちたテスト | App 734 中 |
|------|--------------|-----------|
| m15 開始側だけ落とす(`startValue = f.Value`) | `..._AfterEolConversionToLf_...`(新規)**のみ** | 1 |
| m16 比較側だけ落とす(素の `target.Value` と比較) | `..._AfterEolConversion_WritesEditedCell_...`(T1)**のみ** | 1 |
| m5 両側落とす | 上記 2 本 | 2 |

**どちらか 1 本では片側が無防備なまま残る。** §8.23 の I-3(捕捉側 / 検査側)と**同じ構造**が
EOL 正規化にもあった。

### 8.39 最終レビュー — 「呼出側を見ずに、呼ばれる側だけを見て理由節を書いた」(8 回目・9 回目)

Important-3 と Important-2 はどちらも **通算 8 回目 / 9 回目**の「結論は正しいが理由節が偽」
(§8.3 / §8.7 / §8.15 / §8.21(a) / §8.22 / §8.32 / §8.33 に続く)。レビューが指摘した共通構造を
そのまま記録する:

> 今回の 2 件はどちらも「**呼出側を見ずに、呼ばれる側だけを見て理由節を書いた**」もの。
> Important-3 は `ConvertEols` の中身だけを見て「ガードが無い」と結論し**呼出元を見ていない**。
> Important-2 は `CsvCellEditor` を読み切った一方で**同じメソッドの 17 行下**を見落とした。
> §8.4 の教訓「規則を関数へ括り出したとき、網は関数側だけでなく呼出側にも要る」の
> **doc / コメント版**である。

**手順として足す: 到達可否や不変条件を関数の内側だけで論証しない。必ず呼出側を 1 つずつ当てる。**

### 8.40 最終レビュー M-4 — 生存していた 2 型の変異(行/列取り違え・片方向の関係演算子)

**(a) 行/列の取り違え(`Rows[row]` → `Rows[col]`)が捕捉側・検査側とも全緑で生存していた。**
列テスト 2 本の編集セルが**どちらも `(1,1)` = row == col** で、変異が字面どおり恒等になるため。
編集セルを `(1,0)` へ変えるだけで両方死ぬことを実測した(m17 = `..._InJaggedRow` のみ /
m18 = `..._ColumnCountChanged` と `..._ColumnRemoved`)。**m3 / m13 / m14 の kill も維持**された
(レビューの検算どおり)。CLAUDE.md §4-B「fixture は区別できる形にする」の未適用箇所。

**(b) 形の検査が片方向しか踏んでいなかった。** 行数は**減る** fixture しか無く、列数は**増える**
fixture しか無かったので、等値比較 `!=` を片側の大小比較へ弱める変異が生存する。逆向きを
1 本ずつ足した(`..._RowInserted` / `..._ColumnRemoved`)。実測:

| 変異 | 落ちたテスト | App 734 中 |
|------|--------------|-----------|
| m19 行数比較を `<` へ | `..._RowInserted`(新規)**のみ** | 1 |
| m21 行数比較を `>` へ | `..._RowCountChanged`**のみ** | 1 |
| m20 列数比較を `>` へ | `..._ColumnRemoved`(新規)**のみ** | 1 |
| m22 列数比較を `<` へ | `..._ColumnCountChanged` と `..._InJaggedRow` | 2 |

`..._ColumnRemoved` は `(row, col)` が**同じセル**を指したまま行の形だけが変わるケースで、
M-25 型のデータ破壊ではない。guard が保守的に拒否する側の契約(= 入力を捨てる偽陽性)であり、
これは §4.2 の設計どおりだがコメントにそう明記した。

**測定して分かった冗長性**: `..._ColumnCountChanged`(長方形 × 列が増える)は、
今回測った 19 変異のどれにも**単独 kill を持たない**(m14 / m18 は `..._ColumnRemoved` が、
m22 は `..._InJaggedRow` が同時に落とす)。形を代表する 1 本として残したが、
**冗長性は測定済みの事実として**コメントへ書いた(隠さない)。

### 8.41 最終レビュー — ② 受容にした指摘(網を張らないと決めた)

`StringComparison.Ordinal` → `OrdinalIgnoreCase` の変異は全緑で生存する。**無網であることを
承知で受容した。** 理由:

- 実害が小さい。この変異が通しても書込先は依然「**大小違いの同じ値を持つ実在セル**」であり、
  M-25 が問題にした「別位置を書き潰す」データ破壊にはならない。
- 弁別するには「大小だけが違う値の別セル」を作る fixture が要るが、既存の形テストの主旨
  (行数 / 列数 / 値の 3 節をそれぞれ単独 kill させる構造)を弱めずに差し込む形が作れなかった。

**「網が無い」ことを承知の上での受容であり、`Ordinal` が守られているという主張はしない。**

### 8.42 最終レビュー Minor 群 — 対応一覧

| # | 指摘 | 対応 |
|---|------|------|
| M-1 | テスト側だけが「到達経路は Ctrl+S → ConvertEols の**1 本だけ**」と古い全称を持っていた | §8.32 の「成功 1 + 短絡 3 + 書込失敗 1 = 5 通り」へ更新 |
| M-2 | src 側の到達不能宣言だけが断定形(§8.25 M-3 の留保がテスト側にしか無い) | src にも「T1 / T2 の 2 fixture 分だけが実測、一般には §4.4 の論証」を明記 |
| M-3 | `CommitTargetChanged` の doc が「別物へ変わった / 消えた」の 2 つしか挙げていない | **`csvNow.Ok == false` を独立の第 3 のケース**として追記(§8.19 の L1 実測を引く) |
| M-5 | App 総数が 724 / 730 / 731 の 3 通り | 過去の数値に `(当時)` を付し、現在の表は 734 で統一 |
| M-6 | `CsvCellEditor` の `Begin` / `Commit` の doc が正規化規則を散文で再掲 | `<see cref="CsvWriter.NormalizeEols"/>` 参照へ。規則の持ち主を 1 か所にする Task 1 の目的どおり |
| M-7 | ReadOnly 昇降の 2 コピー(`onCommit` 受理枝 / テストの `MutateBodyWhileEditing`)が `try/finally` を持たない | 両方に追加(`FileController.WriteToPath` と同じ idiom)。挙動不変・734 本全緑のまま |

### 8.43 脆弱性パスの結果と、次ブランチへの申し送り([High] 本ブランチ由来ではない)

**判定 ✅ マージ可。本ブランチ由来の脆弱性はゼロ。**

ただし独立の発見として **[High]「F2 が NUL を含むセルを黙って切り詰める」**が報告された。
**自分で実物を確認し、プローブで実測した**(プローブは記録後に削除。コミットしていない):

- `CsvCellEditor.cs:54` の `Text = field.Value` でセル値を Win32 EDIT(WinForms `TextBox`)へ渡す。
  EDIT は NUL 終端なので U+0000 以降が落ちる(レビューの `:53` は 1 行ずれ)。
- 実測(本文 `a\0b,c2\nd,e` を CSV モードで開き (0,0) を F2 → そのまま `Commit`):

  ```
  PARSE_LEN=3 PARSE=a<NUL>b | HANDLE=True BOXLEN=1 BOX=a | BODY=a,c2<LF>d,e
  ```

  パーサは `a\0b`(3 文字)を保持しているのに、オーバーレイは `a`(1 文字)しか返さず、
  確定で本文が `a,c2\nd,e` になった = **`\0b` が黙って消えた**。
- **M-25 の同一性検証では止められない。** guard が比べるのは開始時と確定時の**パース結果**であり、
  どちらも `a\0b` で一致する。TextBox 由来の確定値は検査対象ではない。
- 攻撃ファイルを要しない: `EncodingDetector.Detect` は **UTF-16 BOM を検出しない**
  (`EncodingDetector.cs:19-20` に「P6 Task 6 で UTF-16 は非対応化」と明記。BOM 判定は UTF-8 のみで、
  最終フォールバックは Shift_JIS)。したがって UTF-16 の CSV は SJIS として復号され、
  **本文が NUL だらけになる**。

**修正には設計判断が要る**(NUL を含むセルの F2 を拒否する / 警告する / オーバーレイを EDIT 以外に
する / 読込時に NUL を弾く)ため、**本ブランチでは直さず次ブランチへ申し送る**。

### 8.44 最終レビュー(L5 チェックリスト)—— 今度は「手順どおり操作すると必ず食い違う」型

`697b8dd` の直後にコード品質パスが L5 チェックリストへ **Important 3 件 / Minor 9 件**を出した。
`20aea6b` の欠陥が「**修正前でも全行 PASS する**」(§8.31)だったのに対し、今回は逆向きで
**「手順どおりに操作すると期待と必ず食い違う」「判定が原理的に取れない」**である。
どちらも**手順を実際に頭の中で 1 手ずつ動かしていない**ことが原因で、型は同じ。

**L5-1(必ず外れる確認行)**: 1-B の確認行に「確定後 `→` で 2 行 2 列を読ませる」と書いたが、
確定後の現在セルは受理枝末尾の `ApplyCell(ed, csv2, row, col, …)` により **(2,0) のまま**である。
`CsvDocument.MoveCell(2, 0, Right)` は `col < Rows[2].Count - 1` の枝で **(2,1) = 3 行 2 列 = `m2`**
(読まれるのは `m2 3行2列`)。**2 行 2 列へ行くには `↑` → `→` の 2 手が要る**
(`MoveCell(2, 0, Up)` = `(1, ClampCol(1, 0))` = (1,0))。手順に `↑` を足し、
`↑` の発声(`1 2行1列`)も確認行へ書いた。

**L5-2(原理的に取れない前提)**: 1-B に前提 B(セル強調が消える)を課していたが**観測できない**。
`CsvCellEditor.Begin` はオーバーレイを `Location = PointFromCharOffset(field.Start)` /
`Width = Math.Max(140, ClientSize.Width/4)` / `Height = Math.Max(LineHeightPx+6, 24)` で置くので、
**1 文字・1 表示行のセルは丸ごと覆われる**。§4 の判定規則(前提が取れない回は PASS にしない)と
組み合わさると **1-B は永久に PASS 不能**だった。

**対象セルを替えても直らない**ことを確かめた: 幅 140px 未満のセルはすべて覆われるので
「1 行セルなら必ず覆われる」。複数表示行のセルにすると「編集対象自身が改行を持たない」という
1-B の存在理由(§8.13 の (b)/(c) 弁別)が消える。**したがってレビューの推奨どおり、
1-B は前提 B を外し、前提 C(`eol-watch.log`)を弁別の主体にした。**
1-B は Esc を挟まないので Enter のあとターミナルへ切り替えられ、C は確実に取れる。
判定表(§4)を「ケース × 前提」の表に変え、**必要な前提がケースごとに違う**ことを明示した。

**L5-3(手順に無い前提)**: 項目 2 の期待が `CSVモード オン …` なのに、手順は開き直すだけだった。
`AppSettings.CsvAutoModeOnOpen` は初期化子を持たない = **既定 false**(§8.34 M-8 で自分が
書いたばかりの事実)なので、既定設定ではモードに入らない。手順 4 に手動トグルを足した。
**同じ文書の中で、自分が 1 つ前の commit で確認した事実を別の箇所に適用し忘れていた** ——
§8.7 / §8.22 / §8.32 が記録した「同じ commit の中で記述が食い違う」型の再発である。

### 8.45 コード側 fixup(`d9c1b0e`)の L5 チェックリストへの反映

**(1) §0.1 の「`ConvertEols` が `ReadOnly=true` のまま本文を差し替える | L3 T1/T2 で固定済み」は
二重に誤りになった。** 実物で確認した:

- production の機構は `FileController.cs:863-874` —— `bool wasReadOnly = doc.Editor.ReadOnly;`
  → `if (wasReadOnly) doc.Editor.ReadOnly = false;` → `try { eolConverted = ConvertEols(…) }`(`:868`)
  → `finally { if (wasReadOnly) doc.Editor.ReadOnly = true; }`。
  **`ConvertEols` にガードが無いことに依存していない**(§8.36 が直した理由節と同じ)。
- L3 も `d9c1b0e` で `MutateBodyWhileEditing`(同じ ReadOnly 昇降)経由へ寄ったので、
  **この行が主張していた形のテストはもう存在しない**。

行を差し替え、§0.2 へ「CSV モードでも保存経路が本文を差し替えられること(= その昇降が
Ctrl+S の実経路で実際に起きること)」を実機で見る対象として置いた。

**(2) 拒否枝の網は 7 本 → 9 本**(`Commit_WhenRowInserted_…` / `Commit_WhenColumnRemoved_…` が追加。
テスト本体の前文も「以下 9 本」へ更新済み)。§0.1 と項目 6 の本数を直した。
**これは §8.34 M-1 と同じ再発で 2 度目**なので、今回は数字の横に
「**本数は網の増減で変わる。引用するときは数え直すこと**」と、数え方(`Commit_When…` 8 本 +
`Commit_WhenRejected_RestoresCellHighlight…`)を書き添えた。**数字だけを書くと必ず腐る。**

**(3) `CommitTargetChanged` の doc が「出す条件は独立に 3 つ」を明記した**((1) 値または形の不一致 /
(2) セル消失 / (3) `Ok == false`)。項目 6 の冒頭をその 3 条件へ書き直し、**(3) も同じく到達しない**
理由(`ConvertEols` は CR / LF 以外に触れないので引用符の対応を崩せず `Ok` を反転できない)を足した。

### 8.46 Task 4 — 項目 1-C(鏡像)を足した判断

`d9c1b0e` が足した `Commit_AfterEolConversionToLf_WritesEditedCell_NotStaleOffsets` の
fixture コメントは「**これは到達不能枝ではない**: 引用符内 CRLF・行区切り LF の CSV を F2 編集中に
Ctrl+S すると `ConvertEols(Lf)` がセル内 CRLF を LF へ書き換えるので、開始側を正規化しないと
**正常な編集が拒否されてユーザーの入力が捨てられる**」と書いている。
**= 実運用で到達する欠陥面が 1 つ増えたのに、L5 側に対応する項目が無かった。**

`mix1c.csv` を足して項目 1-C を起こした。バイトは自分で計算した:

| | 開始 | 編集中 Ctrl+S の後 |
|---|---|---|
| バイト長 | **40** | **39**(1-A / 1-B と逆に**縮む**) |
| CRLF / lone LF | **1 / 3** | **0 / 4** |
| EOL 検出 | `crlf >= lf` が false(1 >= 3)→ `lf >= cr` → **LF** | — |

memo セルは開始 `Start=15 / Length=6`(`"x\r\ny"`)、変換後 `Start=15 / Length=5`。
修正前は `Length=6` を持ち越すので `[15, 21)` = `"x`⏎`y",` を置換し、**区切りカンマまで食って**
2 行目が **`1,NEWt1`**(3 列 → 2 列)になる。**1-A は `length` が 1 足りず、1-C は 1 余る。**

開始側の正規化(m15)が落ちたときの FAIL 様式は「確定が拒否され
`本文が変わったため確定できません。入力は破棄しました` が読まれる」で、
**1-A の同名の FAIL 様式(比較側 = m16)とは原因が別**。どちらか一方の fixture では
他方が恒等になる(テスト側の「完全な鏡像」コメントと同じ構造が L5 にも要る)。

なお §8.43 の [High](NUL でオーバーレイが値を切り詰める)は**本チェックリストに入れない**。
本ブランチの修正対象ではなく、直すには設計判断が要る = 次ブランチの申し送りだからである
(L5 の項目にすると「このブランチで直った / 直っていない」の判定に混ざる)。

### 8.47 最終レビュー(L5)Minor 群 — 対応一覧

| # | 指摘 | 対応(すべて ①。M-9 相当の L5-12 のみ精度修正で受容) |
|---|---|---|
| L5-4 | 前提 D 単体に弁別力が無い(Ctrl+S が握り潰された回でも「`NEW` 無し」は成立) | 期待欄へ「**かつセル内の改行が `0D 0A` になっていること**」を追記し、単体では前提にならないと明記 |
| L5-5 | 前提 B の「⇔」「唯一」が全称として強すぎる | `_cellHighlight = null` の **3 箇所すべて**を grep して表にし、F2 編集中に到達しない論証を書いて**十分条件**へ弱めた(内訳は下記) |
| L5-6 | 項目 3 / 4 も修正前後を弁別しない | 両方に「**退行確認であって M-25 の弁別ではない**」を 1 行。`git diff main...HEAD -- src/kxEdit.App/CsvController.cs` に `onCancel` の本体が現れないことを確認した |
| L5-7 | 1-A 手順 3 の「`x`⏎`y` が全選択される」が項目 7 の注と矛盾 | 「**表示のされ方をそのまま記録する**」へ。`Begin` が入れるのは LF 単独 |
| L5-8 | 符号化劣化の短絡枝は UTF-8 fixture では発火しない | 条件の先頭が `doc.State.Encoding.CodePage != 65001` であることを実読で確認し、「**構造的に発火しない**(短絡評価で `CanEncodeBuffer` すら呼ばれない)」と明記 |
| L5-10 | 監視ループに try/catch が無い | `try { … } catch { continue }`。保存は `AtomicFile` の差し替えなので、**衝突するのは前提 C を記録すべきまさにその瞬間** |
| L5-11 | 「`EscapeField` がセル内改行を `\n` で書く」は帰属先が誤り | `EscapeField` は `,` / `"` / CR / LF を含むかを見て包み `"` を二重化するだけで**正規化しない**ことを実読で確認。揃えているのは `Commit` が呼ぶ `NormalizeEols`。**Task 1 で持ち主を移したのに、設計書 §7 からの転記で移す前の文言が残っていた** |
| L5-12 | 逐語の精度 | `Undo`/`Redo`/`Cut`/`Paste` の**第 1 文は `if (IsComposing) …`** でガードはその次(`Paste` は `return false`)/ `UndoEolConversion` は `conversionRecorded=false` なら何も戻さない / 上限超過用の `_prompt.Error` が `:920` にもある —— 3 点とも実読で確認して直した |

**L5-5 の内訳(自分で grep した 3 箇所)**:

| 箇所 | 呼び出し元 | F2 編集中に到達するか |
|---|---|---|
| `EditorControl.cs:294`(`ReplaceSource`) | App は `FileController.cs:372` の `SetOrReplaceSource`(`LoadInto`)と復元経路 `:1143` / `:1379` / `:1404` のみ | **しない**(読込はダイアログか起動時。ダイアログなら先に `CancelEdit`) |
| `:1045`(`ClearHighlight`) | App 5 箇所。`CsvController.cs:381` / `:387` は **`TryContext` の中**でその第 1 文が `if (_editor.IsEditing) return false;` / `:79`(`TryEnterMode`)・`:106`(`ExitMode` ← `ToggleMode`)はどちらも入口に `_editor.IsEditing` ガード / `FileController.cs:368` は `LoadInto` | **しない** |
| `:592`(`ConvertEols` 非 fast-path) | `FileController.WriteToPath` の Ctrl+S のみ | **する** |

**L5-9 の再判断(束ね方が変わった)**: 「#54 の fixture は `.txt` だから CSV モードに入れない」は
**偽**だった。`CsvController.TryEnterMode` に拡張子判定は無く、見ているのは `doc.ParseCsv().Ok` だけで、
拡張子を見るのは `MainForm.AutoEnterCsvMode`(**自動進入**)だけである。**結論(fixture は共有できない)は
維持だが、理由が全部入れ替わった** —— (1) 1-A / 1-C は「引用符で囲まれたセル内改行を持つセル」を要求し
#54 の fixture にそれが無い、(2) 混在 EOL は Ctrl+S 1 回で消えるので **1 ファイルは 1 観測にしか使えない**。
**束ね方には新しい可能性が 1 つ出た**: #54 の `mixed-eol.txt` を CSV モードに入れれば
**1-B 型(オフセットずれのみ)の観測は原理的に可能**である。ただし #54 の ② がその混在 EOL を先に
消費するので、束ねるなら**どちらがその 1 回を使うかを決める**必要がある —— §5 に書いた。

**総括**: docs だけの fixup 2 回で「結論は正しいが理由節/射程/手順が偽」が通算 **11 件**
(§8.32 / §8.33 の 2 件 + 今回の L5-1 / L5-2 / L5-3 / L5-5 / L5-8 / L5-9 / L5-11)。
**今回の 3 件(L5-1 / L5-2 / L5-3)に共通するのは「手順を 1 手ずつ実際に動かしていない」こと**である。
コードの理由節と同じく、**手順も「書いたら、頭の中で 1 手ずつ実行して期待値を導き直す」**。
