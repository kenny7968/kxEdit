...
> 【Task 5 仕様レビューからの申し送り 2026-09-15・**Task 7 の必須確認項目**】
> **`_activate(form)` の呼び出しを `try`/`catch` で囲むこと。** 2 箇所ある:
> `Request()` の `BeginInvoke` のラムダ内と、`Attach()` の直接呼び出し。
>
> `WindowActivator.Activate` は `form.Handle` で **`ObjectDisposedException` を投げうる**
> (実測)。終了中レース —— `BeginInvoke` 成功後・デリゲート実行前に `MainForm` が
> 破棄される —— で踏む。包まないと次が起きる(実測):
>
> - WinForms は `BeginInvoke` のデリゲート内例外を `Application.ThreadException` へ回送する
> - `Program.cs` の配線が `CrashHandler` → `Environment.Exit(1)` を呼ぶ
> - 一方 `WaitAny` は 0(完了)を返すので `Request()` は **`true` を返し ACK を送る**
> - 結果、**B は「引き渡し成功」と思って静かに終了し、A だけが死ぬ**
>   = ユーザーから見ると「2 つ目を起動したら 1 つ目が消えた」
>
> `Attach()` 側は直接呼び出しなので、包まないと `OnShown` を巻き込んで**起動を止める**
> (設計の「前面化の失敗で起動を止めない」に直接抵触する)。
> なお対処を `WindowActivator` 側のガードにしないこと —— あちらは自動テストの網の外で、
> 検証不能な分岐を増やさない方針(設計 §5)。
...
既存の `docs/plans/2026-09-*-l5-checklist.md` の書式に合わせる。

> 【Task 5 仕様レビューによる拡充 2026-09-15】策定時は 8 項目だったが、レビューが
> scratchpad の使い捨て WinForms プロジェクト 5 本で実測し、**抜けていた観点を洗い出した**。
> 特に L5-4(モーダル中に最小化)は、Task 5 の Critical 欠陥を踏む唯一の項目である。

> 【重要 2026-09-15】**L5 の実施には下の表ではなく
> `2026-09-14-single-instance-l5-checklist.md`(30 項目)を使うこと。**
> 下の表は策定〜Task 5 時点のスナップショットで、その後 Task 7 の手動確認と
> Task 7 仕様レビューでさらに観点が増え、`ForeignPeer` の文言も訂正された。
> 特に**「2 つ目はスクリプトではなく Win+R 等の実ユーザー操作で起動する」**という
> 最重要の前提は下の表には書かれていない(スクリプト起動だと前面化だけが失敗し、
> 動いている機能を FAIL と誤判定する)。

| # | 手順 | 期待 |
|---|---|---|
| L5-1 | A を**通常サイズ**で最小化 → もう一度起動 | 元の位置・サイズで復帰して前面化。B は無言終了。SR が A のタイトル/タブを読む |
| L5-2 | **A を最大化 → 最小化** → 起動 | **最大化状態で復帰する**(通常サイズに落ちないこと)。`SW_RESTORE` を使う理由がこれ |
| L5-3 | A で設定ダイアログを開いたまま → 起動 | **設定ダイアログ自身**が前面化し**キー入力を受け付ける**。ビープが鳴らないこと。SR がダイアログ名を読む |
| **L5-4** | **A で設定ダイアログを開く → `Win+D` で最小化 → 起動** | 同上。**Task 5 の Critical 修正の検証点**。修正前はここで「前面には来るがビープのみ」になる |
| L5-5 | 設定ダイアログ → フォント選択ダイアログ(入れ子モーダル)を開いたまま → 起動 | **最も内側**のダイアログが前面化し入力できる |
| L5-6 | 「開く」「名前を付けて保存」の**共通ダイアログ**を開いたまま → 起動 | 共通ダイアログが前面化し入力できる。※レビューの実測はマネージド `Form` のみ。**共通ダイアログは未検証なので必ず実機で見ること** |
| L5-7 | 上書き確認 MessageBox を出したまま → 起動 / さらに `Win+D` 後に起動 | 前者は MessageBox が前面化。**後者は owner が付かない可能性があるので実機確認必須** |
| L5-8 | **非モーダルの検索/置換ダイアログ**にフォーカスを置いたまま → 起動 | 検索ダイアログが前面化する。**これが望ましい挙動かはユーザー判断が要る** |
| L5-9 | **grep 結果ウィンドウ**にフォーカスを置いたまま → 起動 | 同上 |
| L5-10 | A で**メニューを開いたまま** → 起動 | メインフォームが前面化する(メニューは拾われない)。メニューが固まらない・SR の読みが壊れない |
| L5-11 | 引き渡し直後の**フォーカス位置** | `MainForm.OnActivated` が編集領域へフォーカスを戻す実装がある。前面化先がモーダル/非モーダル窓のとき、そこからフォーカスを奪わないこと |
| L5-12 | `kxEdit.exe --new-instance` | 2 つ目が独立に起動し、**A を前面化しない** |
| L5-13 | A を**別の仮想デスクトップ**に置いて → 起動 | デスクトップが切り替わって前面化するか、少なくとも操作不能にならない |
| L5-14 | **昇格差**: A を管理者で起動 → 非管理者で 2 つ目(および逆) | UIPI で `AllowSetForegroundWindow` / `SetForegroundWindow` が無言で落ちないか。落ちるならタスクバー点滅で気づけるか([[nvda-harness-breaks-if-nvda-restarts-elevated]] と同型の罠) |
| L5-15 | A が**フルスクリーンの別アプリの背後** / **別モニタ**にある | 前面化が拒否されたときタスクバーボタンが点滅するだけで終わるか(設計どおりの degrade) |
| L5-16 | A の**起動直後**(ウィンドウ生成前)に 2 つ目を起動 | `PendingActivation` の保留経路。A の起動が完了して前面に出る・B はエラーを出さない |
| L5-17 | A でファイルを開き未保存にする → 2 つ目を起動 | 未保存内容が失われない。バックアップが消えない |
| L5-18 | 別フォルダにコピーした kxEdit.exe を起動 | ~~「別の場所にある kxEdit が既に起動しています」が出る~~ → **文言は訂正済み。`2026-09-14-single-instance-l5-checklist.md` の項目 19 を参照すること**(この表の文言では絶対に出ないため、そのまま使うと正しい実装を FAIL と記録してしまう) |
| L5-19 | 全部終了 → すぐ起動 | 普通に起動する(「応答しません」が出ない=終了中レースの取りこぼしが無い) |
