...
- **S-9**(Task 6 で判明・2026-08-23。**Task 6 の仕様レビューで見立てを訂正**): **A-7 (b) の修正は
  ...
  **バックアップ Id のみ**で行われパスでは行われないため(`FileController.cs:781-783`)、
  ...
  `BackupCoordinator` ではないので §6 の非目標には当たらない)。
...
- **S-10**(Task 6b で判明・2026-08-23): **クローズ/終了が黙って中止される。**
  未保存確認で「はい」→ `SaveDocument` が重複タブガードで false → `keepClosing=false` →
  `e.Cancel = true`。エラーは読み上げられるが、**その後「終了しなかった」ことを伝えるものが無い**。
  SR ユーザーはアプリが閉じたと思い込みうる。修正には `MainForm` のクローズループに触れる必要が
  あり Task 6b の範囲外。Task 7 / 8 が保存フローを触るときか、follow-up で回収する。
