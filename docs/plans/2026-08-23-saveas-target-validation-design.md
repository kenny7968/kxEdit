...
- **S-13**(Task 8 で判明・2026-08-23): **上書き承諾が周回をまたがないこと(§10.11)に
  ...
  caption をキーにした応答テーブル(`Dictionary<string, bool>` + 既定値・6 行程度)を
  `FakePrompt` に足せば表現できるが、**最後のコードタスクで共有テストシームを導入する**ことになり、
  CLAUDE.md §3 の「後続タスクが依存する新しい抽象・seam を導入する → コード品質レビュー」に
  該当する。本ブランチでは足さず記録に留める。**現状の唯一の網は L5 ⑤ の後半**
