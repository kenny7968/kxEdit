...
- **S-8**(Task 5 の fixup で発生・2026-08-23): V-1 の修正 (b) で `WriteToPath` の catch フィルタに
  ...
  **最終ブランチレビューの判断(2026-08-23): ② 現状維持 + 根治は `AtomicFile` へ retarget して follow-up。**
  `AtomicFile.Write` の呼出元は `BackupStore.Write` / `SessionLayoutStore.Save` /
  `TextFileService.Save`(2 形)の 4 つ。**バックアップ書込の例外型を変える**ことになり、
  `SerialBackupWriter` の `OnWriteFailed` 経路とクラッシュ安全性に波及するので、
  最終レビューの fixup で入れる変更ではない(独自の設計 + 2 パスレビューを要する)。
  現状の拡幅は境界が限定的で、`WriteToPath` のフィルタは既に `NotSupportedException` /
  `SecurityException` も握っており**そもそも純粋な I/O フィルタではなかった**。前段ガードがある以上、
  `ArgumentException` は 2 本の細い経路(復元タブの Ctrl+S / 将来の新規呼出元)への防御深度として妥当。
  **1 点訂正**: S-8 が引用した「想定内の入出力エラーのみ握る。NullReference 等のロジックバグは
  伝播させる」というコメントは `WriteToPath` ではなく **`LoadInto` の catch** にあり、そちらも
  main の時点から `ArgumentException` を握っている。つまりこのコメントは**両方のフィルタに対して
  不正確**。
...
- **S-16**(最終レビュー品質パスで発見・2026-08-23): **書込失敗だけがダイアログへ戻らない。**
  §4.5 は「ダイアログの中で選んだ値への警告なら、そのダイアログへ戻す」を全体の原則として掲げ、
  空白パス・正規化失敗・重複タブ・到達不能・上書き辞退・劣化警告の 6 つを従わせた。しかし
  **フォルダー名のタイプミス**(`<tmp>
o-such-dir.txt`)は `TryInspectSaveTarget` の
  ローカル枝を素通りし、`WriteToPath` の中で初めて失敗して `return false` する。実測で
  `pickCount = 1` = 再表示されない。**60 文字のパスを打ち直させる。**
  しかもこれは §10.2 自身が「SR ユーザーが直入力でタイプミスした典型ケース」と名指しした当のケース。

  §4.1 の擬似コードは `失敗ならロールバックして return false` と書いているので**実装は計画どおり**で
  逸脱ではない。ただし**なぜ書込失敗だけ原則の外なのかがどこにも書かれていなかった**ので記録する。

  **② 受容の理由**: §4.5 の原則は「**ダイアログで選んだ値の検証**」に対するもので、書込の失敗は
  検証ではなく I/O の結果である(ディスクフル・AV ロック・権限は打ち直しでは直らない)。
  一方で**タイプミスの場合だけは打ち直しで直る**ので、この線引きは完全には切れていない。
  `return false` → `continue` にすればロールバック済みで `seed` が失敗パスを保持しているため
  再表示は自然に動くが、**両方のレビューパスが完了した後の挙動変更**になり、それ自体が
  別エージェントレビューを要する(CLAUDE.md §3)。**follow-up の最有力候補**として残す。
  採るときは L5 に 1 項目追加が要る。
- **S-17**(最終レビュー品質パスで発見・2026-08-23。**既存 / 本ブランチが主経路化した**):
  **書込失敗の文言が内部一時ファイル名を読み上げる。** 実測:
  `保存できませんでした: Could not find a part of the path '...
o-such-dir.txt.silk1rmf.j15.tmp'`。
  `AtomicFile` のステージング名(`<名前>.<乱数>.tmp`)が `ex.Message` 経由でそのまま出るので、
  SR では意味のないランダム 8 文字が読み上げられ、しかも**ユーザーが指定していないファイル名が
  主語になる**。本ブランチが作った欠陥ではないが、A-7 (a) / A-19 が「テキストボックス直入力」を
...
> **追記(最終レビュー品質パス I-5・2026-08-23): このメンバーは S1006 の網に依存しなくなった。**
> `defaultCancel` から既定値 `= false` を落として**必須引数**にした
> (`IUserPrompt` / `MessageBoxUserPrompt` / `FakePrompt` の 3 箇所)。
> 全リポジトリの呼出 2 箇所はどちらも `defaultCancel: true` を明示しており依存はゼロ。
> 動機は「既定値が無害な方(OK 側=危険な方)を指していた」こと: S-12 の教訓は
> 「破壊的な確認では安全側の既定を選ぶ」だが、それを**散文の約束**として `IUserPrompt` の
> doc に書いていた。既定値を消すと、将来の `OkCancel` 呼出は**コンパイラが側を選ばせる**ので
> 約束が機構になる。S1006 は「実装が interface の既定値から逸れないこと」の網であって
> 「既定値そのものが安全側を向いていること」の網ではないので、そもそも今回の欠陥は
> S1006 では捕まらなかった。§10.3b / 本節のアナライザ活用そのものは有効なまま。
...
### 10.16 最終レビュー品質パスの結果(2026-08-23)

**Critical ゼロ。条件付きマージ可。** マージ前の処置は I-1 / I-3(どちらも `src` を触らない
テスト追加)の 2 件のみ、という判定。

#### §10.9 の台帳に 1 行追加 — `MessageBoxUserPrompt` の安全側マッピングも kill 不能

`MessageBoxUserPrompt.cs` の `defaultCancel ? Button2 : Button1` を `Button1` に固定しても
**全緑・かつ `-warnaserror` で警告ゼロ**(interface 実装の引数なので S1172 も出ない)。
S-12 は「`defaultCancel` は `FakePrompt` に記録できるので安全側の既定が L3 の assertion になる」と
書いたが、正確には**半分**:

- **決定**(`FileController` が `true` を渡すこと)= 網あり
- **効果**(MessageBox が実際にキャンセルへフォーカスすること)= **網なし**

`SaveAsDialog.OverwritePrompt = false`(§10.9)と同じ枠で、**L5 ① / ⑤ が唯一の網**。

#### §10.4 の等価変異台帳に 1 行追加

`Reachable = fileExists || dirExists` から `fileExists ||` を落とす変異は **Windows 上で等価**。
`File.Exists(p)` が真なら親ディレクトリは必ず存在するため観測可能な差が出ない。
`&&` へ変える変異が `UnderMissingDir` で kill されるのとは別物なので混同しないこと。

#### `SaveAsDocument` を分割すべきか — 却下(理由を残す)

6 段 156 行(実コード 94 行)だが**抽出すべきでない**、というのが品質パスの判定で、これに同意する。

1. **6 段すべてが「ループを回し直す」ことを帰結とする。** 段をメソッドへ出すと、戻り値を
   3 値(続行 / 再表示 / 中止)か enum にするしかない。すると現在 `continue` という**文法**で
   表現されている決定が、静かに書き換え可能な**値**になる。これは §10.3 が実測した劣化
   (`&&` の短絡に構造的に埋め込まれていたフェイルセーフを 1 トークンの引数へ移したら
   変異が全緑で生存した)と**同型**であり、しかも §10.3b のとおり現在は S1854 が
   「`continue` であること」をコンパイル時に固定している。抽出はその信号も失わせる。
2. 4 つのローカル(`seed` / `picked` / `full` / `doc`)を段が共有する。
3. **段の順序自体が仕様**(§4.2)。1 本のメソッドに並んでいることが順序の可読性そのもの。

`SaveDocument` / `SaveAsDocument` に重複する述語(`FindByPath` + `!ReferenceEquals`)の抽出も
**述語だけ**に留めるべきで、ブロックごと共有すると 2 つの文言が合流し、
「Ctrl+S 側で『そのタブで保存してください』を読み上げる」という
`Save_PathAlsoOpenInAnotherTab_...` が `DoesNotContain` でわざわざ防いでいる事故を招く。現状維持。
