...
1. **§9.8 の副産物は再現しない。** fixture は純 ASCII(小文字 a-z のみ・500,000 バイト・
   改行 0)で、実機で開くと **0.41 秒**で完了し一度も応答不能にならなかった(§2.4)。
   240 秒が出た原因は特定できていない。**「長大 1 行ファイルは開けない」という §9.8 の
   記述は事実に反する。** §9.8 自体は策定時スナップショットとして触らず(CLAUDE.md §8)、
   訂正は本書に記録する。
...
(§2.3)。また **F-6 の申し送りが推定から実測になった**(§2.2)。
...
| ID | 調査前の判断 | 調査後 | 根拠 |
|---|---|---|---|
| 副産物 | Yes(規模は調査後) | **取り下げ** | 再現しない(§2.4)。原因未特定のまま閉じる |
| F-5 | Yes | **保留(未確認)** | 実機実害を測れていない(§2.4)。Core 単体の実測値はあるが、SR / UI が実際に固まるかは未検証 |
...
- **N-3: F-5 の実機実害が未確認**
  - `Responding` ポーリングで測ったが、**キーがエディタへ届いたかを確認していない**
    (§2.4)。キャレット位置を UIA で読むなどして到達を確認する測定に作り直す必要がある
- **N-4: F-3(UIA 単語境界のずれ)の実害採取が未実施**
  - NVDA スピーチビューアーを UIA で読む手法(PR #33 §9.8)で採取する
- **N-5: §9.8 の 240 秒の原因は未特定**
  - 再現しないため追跡は打ち切る。再発したら `Responding` だけでなく
    **キャレット位置やウィンドウ状態も併せて記録する**測定にする
