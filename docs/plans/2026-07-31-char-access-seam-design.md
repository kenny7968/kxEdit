...
### 9.8 L5 実機 SR 検証(NVDA スピーチビューアー経由・2026-08-02)

**手法**: NVDA を実際に動かし、**スピーチビューアーの内容を UIA で読み取って**発話内容を
テキストで確認した(`RICHEDIT50W` ペインの `Name` プロパティに全文が入る)。
NVDA 本体は本物が動いているため「何を読み上げるか」は実測できる。
**音として出ているか・点字出力・体感の引っかかりは対象外。**

#### 確認できた項目

| 項目 | 操作 | NVDA の発話 | 判定 |
|---|---|---|---|
| ② サロゲート | `a😀b` の 0 から → | 「にっこり笑う」 | **1 文字として読む** |
| 〃 | 続けて → | 「ビー」 | **一手で b へ・壊れた片割れを読まない** |
| ① CRLF | `abc\r\ndef` で End | 「フッキ カイギョウ」 | **1 キャレット位置・1 発話** |
| 〃 | 続けて → | 「デー」 | 一手で d へ |
| 〃 | 続けて ← | 「フッキ カイギョウ」 | **LF 単独ではなくペア全体・逆方向も atomic** |
| ③ 単語ナビ | 1MB 文書で Ctrl+← | 「verwe」「jksomsvb」 | 単語単位で読む |
| ⑤ 大きい文書 | 1MB を開く → Ctrl+End | 最終行を正しく読む | フリーズなし |
| 〃 | Ctrl+← × 40 連打 | 395 ms(9.9 ms/回) | 追従する(発話は中断される=SR の正常動作) |

**①② は本作業の中心的な主張(規則の重複解消)が実発声で裏づけられたもの。**
特に CRLF が「1 つのキャレット停止で 1 発話」になっている点は、2026-07-24 の設計書 §10 F-1
(「L5 で別読みされる場合のみ正規化 fixup を検討」)が不要だったことの再確認でもある。

#### 未検証

- NVDA レビューカーソル(テンキー)での文字単位・単語単位読み
- 上書きモードでの発話
- 音として出ているか / 点字出力

#### ★ 副産物: 長大 1 行ファイルは**開いた時点で**ハングする(F-5 の強化)

500KB の空白・改行を一切含まない 1 行ファイルを開くと、**UI が 240 秒経っても応答しない**。

**同じファイルで `main`(本ブランチ着手前)をビルドして比較したところ、まったく同じく
ハングした。** したがって本作業が原因でも、本作業で解消されたものでもない**既存問題**である。

測定上の注意として記録する: 最初この経路を `SendKeys` の所要時間で測って
「10.5 ms / 11.5 ms」という値を得たが、**これは無効**だった。`SendKeys` はハングした
ウィンドウのメッセージキューに投げて即座に返るため、処理完了を待たない。
`Process.Responding` のポーリングで測り直して判明した。

F-5 は「単語ナビが遅い」と書いたが、実態は**ファイルを開く時点で固まる**方が重い。
主因は §8 F-5 に併記した「折り返し ON × 改行なし巨大 1 行」(論理行全体の `GetText` と
`LineLayout.Wrap`)と見られ、文字アクセスの高速化では解消しない。
