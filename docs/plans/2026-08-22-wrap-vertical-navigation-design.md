...
| ID | 内容 |
|----|------|
| S-6 | **`OnPaint` での `_topSegment` 自己修復案は却下した**。`ViewportLayout.Build` が返す `rows[0].SegmentIndex` を `_topSegment` へ書き戻せば陳腐化(編集で先頭段落が縮んでも `_topSegment` をリセットしない設計=§4.1)が無料で消える、という案が Task 3 の品質レビューで出た。却下の理由は 2 つ。(a) **描画中に状態を書くのは再入・順序の観点でリスク**(`Invalidate` を誘発する経路との相互作用、`OnPaint` が UI スレッド以外から来ないという前提への依存が増える)。(b) **残る実害が小さい**=陳腐化は `AfterEdit` → `BringCaretIntoView` の第 1 分岐(辞書順比較)が修復する。ただし**修復するのはキャレットが `_topLine`(またはそれより上の論理行)にある場合に限る**。キャレットが `_topLine` より**後**の論理行にあると第 1 分岐は発火せず(`caretLine < _topLine` も `caretLine == _topLine` も偽)、第 2 分岐も `CountVisualRowsForward` が陳腐化した起点セグメントを最終セグメントへ寄せて数えるため「可視」と判定する=陳腐化した `_topSegment` は**そのまま残る**。残った場合の帰結は限定的で、描画は `ViewportLayout.Build` のクランプにより破綻せず、`WalkBackVisualRows` がクランプしないぶん**ホイール上方向が空振りしうる**だけである(空振り回数の上界は同メソッドの remarks 参照=陳腐化の超過分に比例し上限は無い)。**将来 `BringCaretIntoView` の呼び出し構造を変えるとき(S-8)は再検討の価値がある**=第 1 分岐が(この限定付きで)自己修復を担っている事実が前提になっているため。**2026-08-23 訂正**: 当初この欄は「編集直後に必ず修復するため 1 フレームを跨いで残らない」と無条件に書いていたが、最終レビュー品質パスの probe で反証された。src 側 remarks(`EditorControl.Caret.cs` の `BringCaretIntoView`)は当初から「キャレットが先頭論理行にある限り」と正しく限定しており、**本欄だけが限定を落としていた** |
...
| S-10 | **SR の `ScrollIntoView` 1 回あたりのコストが「論理行 1 本の文書」で 0 → 2〜17 ms になる**(最終レビュー脆弱性パス Medium)。`ScrollCharRangeIntoView` は対象論理行が `[TopLine, TopLine+visibleRows)` の外なら粗い否定で Wrap を省くが、**論理行が 1 本しかない文書ではこの否定が原理的に成立し得ず**、SR がレビューカーソルを 1 単位動かすたびに `LocateVisualRow`(論理行全文の `GetText` + `WrapThroughOffset`)を払う。E-1 の再現ファイル(CJK 500K 単一行)がまさにこれ。**§5 の「打ち切れない箇所は 2 つ」の列挙にこの経路が抜けている**(策定時の見落とし)。実測(CJK 500K・`WrapColumns=80`・可視 30 行): 既に可視 2.1 ms(depth 0)/ 4.1 ms(1000)/ 約 8 ms(5000)、スクロールを伴う場合 5.5 / 8.2 / 15.6〜17.3 ms。深さ非依存の 2 ms はほぼ 500K char ≒ 1MB の string 化(LOH)。main は単一論理行では必ず「既に可視」と判定して Wrap ゼロだったので、**純粋な増分**である。ハング(PR #35 の 40 秒級)は実証されておらず受容したが、消すなら `UiaTextHostAdapter._lastLineSegs` と同型の 1 エントリメモに `LocateVisualRow` を載せれば足りる(SR は同じ `(snap, line, wrap)` を連打するので命中率が高い)。**L5 ⑨ で実機の体感を確認する** |
| S-11 | **不変条件 I-3 の証拠は「振る舞い」だけを支えており「コスト」は支えていない**(最終レビュー品質パス)。§2 の I-3 は「すべての式が変更前と同一に退化する(**振る舞いもコストも**)」と書き、その証拠を「折り返し OFF の既存テストが 1 行も変わらず全緑であること」に置いている。しかし最終レビューで、折り返し OFF の 4 つの短絡(`LocateVisualRow` / `SegmentCountCapped` / `ScrollByVisualRows` / `ScrollCharRangeIntoView`)は**すべて等価変換であり値ベースのテストでは kill 不能**であることが確定した。したがってテストが証明しているのは**振る舞いの不変だけ**で、**コストの不変を守っているのはレビューとコメントだけ**である。実際 Task 3 で `LineTextOf` による OFF 側の行全文 string 化が入り込み、テストではなく品質レビューで発見されている(実装計画 §3 Task 3 (c))。**§1〜§7 は策定時スナップショットなので本文は直さず、I-3 の読み方をここで限定する**: I-3 の「コスト」側は不変条件として掲げるが、その**証拠は自動テストではない**。コストにも網を張るなら `GC.GetAllocatedBytesForCurrentThread` ベースの alloc 検査が筋だが、本リポジトリに前例が無く導入判断を伴うため**別テーマ**とする |
