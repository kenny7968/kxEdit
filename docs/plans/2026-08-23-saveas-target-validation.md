...
## Task 6b: A-7 (b) — Ctrl+S 側にも重複タブ検査を置く

> **Task 6 の仕様レビューで追加(2026-08-23)。設計書 §9 S-9 を参照。**
> Task 6 は重複タブが**生まれる**経路を塞いだが、**既にある**状態では
> `SaveDocument` → `WriteToPath`(Ctrl+S)が `FindByPath` を参照しないため無警告の上書きが残る。
> しかも Task 6 は非対称を作った: 衝突した 2 タブのうち**新しい方は自分のパスへの SaveAs が
> 誤検知で塞がれる**のに、**Ctrl+S は通って他方を破壊する**。安全な操作が塞がれ、
> 破壊的な操作が通っている状態なので、これを残したまま「A-7 (b) を直した」とは言えない。

**Files:**
- Modify: `src/kxEdit.App/FileController.cs`
- Modify: `tests/kxEdit.App.Tests/FileControllerTests.cs`

### Step 1: 置き場所を決める(先に調べる)

`WriteToPath` の呼出元を全列挙し、**`SaveDocument` に置くか `WriteToPath` の冒頭に置くか**を判断する。
既定は **`SaveDocument`**(Ctrl+S の入口。`WriteToPath` は低レベルの書き込みプリミティブで、
そこに UI ポリシーを足すと層が濁る)。ただし `SaveDocument` を迂回して衝突パスへ書ける経路が
あるなら報告すること。判断と根拠を報告に書く。

**プローブより前に置くこと。** 重複は保存させないので到達性を調べる意味がなく、
遠隔共有で無駄な 5 秒を待たせない(Task 6 と同じ理由)。

### Step 2: 失敗するテストを書く

fixture の作り方: **`SaveAsDocument` を通さずに衝突状態を作る**(Task 6 のガードが邪魔をするため)。
`Save_ExistingPathIsDriveRoot_ReportsError_AndRollsBackModified` が使っている
「`State.Path` を直接代入する」手法と同じ。2 タブとも同じ実ファイルを指すようにする。

網を張る先:

1. **新しい方の Ctrl+S が止まり、ファイルが書き換わらないこと**(本体)。
   古い方の内容がディスク上に残っていることを実ファイルで assert する。
2. **古い方の Ctrl+S は通ること**(過剰検知の防止)。**この対照群が無いと、
   「常に false を返す」変異が生き残る。**
3. **衝突していない通常のタブの Ctrl+S が通ること**(既存テストで足りるなら新規不要。
   足りるかどうかを確認して報告する)。

### Step 3: 実装

Task 6 と同じ述語(`_docs.FindByPath` + `!ReferenceEquals`)を使う。**文言は Task 6 と変える**:
Task 6 は「そのタブで保存してください」と言えるが、Ctrl+S では**呼び出し元のタブ自身も
そのファイルを持っている**ので、その案内は成立しない。行動可能な指示にすること
(例: 「別の名前で保存してください」)。

### Step 4: 緑を確認してコミット

`fix(app): 同一ファイルを開く 2 タブ目の Ctrl+S を止める(A-7 (b) 残余)`

### Step 5: ミューテーション

- 新ガードを除去 → テスト 1 が kill すること
- `!ReferenceEquals(other, doc)` を除去 → テスト 2 が kill すること(**対照群の意味**)
- Task 6 のガードを除去 → Task 6 のテストが kill すること(Task 6b が Task 6 の網を
  空虚にしていないことの確認)
