...
`変更履歴.txt` の**過去のエントリは書き換えない**。過去リリースの記述を後から改変すると
履歴が嘘になる。先頭に新エントリを足し、改名の事実と `%AppData%` の非移行を明記する。

(2026-08-21 実施時の訂正: 当初この理由を「実在した zip 名を改変すると」と書いていたが、
実ファイルに zip 名は含まれていなかった。除外の結論は変わらない。)
