...
以下は実施後(2026-08-21)に追記した申し送り。

- **`check-no-local-paths.ps1` の両対応 regex の撤去** — `(yEdit|kxEdit)` は
  ローカル作業ディレクトリが旧名のままでも検出を効かせるための**暫定措置**である。
  ユーザーが作業ディレクトリを `kxEdit` へ rename したら `yEdit` の枝を落とす。
  それまで残す。落とし忘れても実害はない(検出範囲が広いだけ)
- **説明書への読みの再掲** — 3 行目の `(ワイエディット)` は、L5 で NVDA の実際の読みが
  確定するまで表記と発声が食い違うのを避けるため、ユーザー判断で削除した。
  L5 で読みが確定したら `kxEdit(◯◯)` の形で戻すか判断する。
  説明書はユーザー編集版が正なので、こちらから改稿しない
