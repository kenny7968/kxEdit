...
> **追記(最終レビュー 脆弱性パス L-1)**: 本節が挙げていなかった具体例として
> **「AV / DLP が保存中の宛先を隔離削除 → kxEdit が黙って書き戻し『保存しました』と応答」**がある。
> セキュリティ製品の隔離アクションが、その 1 書込に限り無効化される。**受容する**:
> リアルタイムスキャナは直後に再度隔離するし、修正前もユーザーが手で保存し直せば同じ結果に至る
> (= 到達可能な状態は増えていない)。差分は「無言である」ことだけで、それは本節が既に受容している。
> 後日の再監査でこの具体例を拾い直せるよう、ここへ残す。
...
**`TryQuarantineCorrupt(path, status, …)` にして構造的に封じる案は採らなかった。**
理由は「その形が成立しないから」ではなく、**現状の呼出数では位置と網で十分だから**である
(呼出 1 か所・網あり・メソッド名 `TryQuarantineCorrupt` 自体が前提を宣言している)。
構造化する場合は戻り値を `bool` ではなく「退避した / 対象外 / 失敗」の 3 値にして、
**対象外と失敗を呼出側が区別できる**ようにすること。呼出が増えたら再考する。

> **訂正(§10.17 指摘 1)**: 本節の初稿と xmldoc は、退けた理由として
> ①「App 層の判定を Core の API 形状へ持ち込む」②「status 違いの no-op と退避の失敗がどちらも
> `false` になり区別できない」の 2 つを挙げていた。**どちらも成立しない。**
> ① `SettingsLoadStatus` は `kxEdit.Core.Settings` の型(`src/kxEdit.Core/Settings/SettingsLoadStatus.cs`)で
> **App 層の判定ではない**。② は**戻り値を `bool` に固定した場合にのみ**成立し、3 値の enum にすれば
> 区別できるので構造化案そのものを否定する論拠にならない。**結論(構造化しない)は脆弱性レビューでも
> 妥当と判定された**が、根拠が偽だったので上記へ差し替えた。xmldoc も同じ内容へ直してある。
> §10.1 以来の「結論は正しいが理由節が偽」で、本ブランチ 5 度目。
...
> **訂正(§10.16 指摘 4)**: 本節の初稿は見出しを「5 件・1 件生存」、commit `31351ad` の本文を
> 「変異 5 件を実測し 4 件を撃墜」と書いていたが、**下の表は 6 行で 5 殺 1 生存**である。
> M3a と M3b を 1 件に数えても「4 件撃墜」は M3a の撃墜を落とす。**表が正しい。**
> commit は書き換えずここに補正を残す。なお M3b はその後**撃墜済み**(§10.16 指摘 1)。
...
#### ~~網にできなかったもの —— 上限 260 の切り詰め(M3b)~~ → **撤回**(§10.16 指摘 1)

> **この節の主張は誤りだった。網は張れる。**当時の記述は「260 超の fixture は MAX_PATH を
> 越えるパスにファイルを作る必要があり、**この開発機では作れたが長パスが有効だから**で、
> CI で成立する保証が無い」というものだったが、**括弧内が偽**である ——
> レビュアー実測で `LongPathsEnabled` は**この開発機で未設定**(既定の無効)であり、
> 275 文字が作れたのは **.NET ランタイムが `\\?\` を自動付与する**性質による(OS 設定に依存しない)。
> したがって「CI で成立しない」懸念は根拠を失う。fixture を 275 文字へ引き上げて **M3b は撃墜済み**。
> 経緯と実出力は §10.16 指摘 1。当時の記録なので本文は残し、ここに撤回を明示する。
...
(2) 実装後に上表の変異 6 件を当て、5 件が赤になることを実測(当時 M3b のみ生存 → §10.16 で撃墜)。

### 10.16 Task 8 — 仕様レビューの反映(張れる網を 2 本張り、観測できない主張を撤回する)

仕様レビューは**要求 1〜6 すべて充足**と裁定し、指示が名指しした 5 種の変異(suffix / 引数順 /
分岐入替 / パス削除 / `return true`)は**全部殺せている**ことを独立に再現した。§10.15 の主要な判断も
裏付けられている:

- **「ユーザーが設定を変えなくても上書きされる」は事実。**レビュアーは `MainForm.cs:594` が
  `OnFormClosing` 内にあり早期 return は再入ガードとユーザーキャンセルの 2 つだけであること、
  しかも直前(`:576-578`)で `WindowWidth` / `WindowHeight` を代入してから保存するので
  **何も操作しなくても終了だけで書き直される**ことまで確認した。
- **`RegisterRecent` の呼出経路は当方の申告より 1 本多い**(`FileController.cs:276` = 既に開いている
  ファイルへの切替 fast-path)。つまり §10.15 (1) の主張は**弱すぎることはあっても過大ではない**。
- `FileShare.Delete` の性質と M1 の撃墜も独立に再現され、セルフレビューの修正は本物と評価された。

以下は 4 指摘の反映。**分類・文言・実装はどれも変えておらず、直したのは網 2 本と記述 2 か所である。**

#### ★ 指摘 1 —— 「網が張れない」が事実として誤りだった(最重要)

§10.15 は M3b(`OneLine(quarantined, 260)`)を「張れなかった」と書いていた。理由として挙げたのは
「275 文字が作れたのは**この開発機で長パスが有効だから**で、CI で成立する保証が無い」。
**この理由節が偽である。**レビュアー実測:

- `HKLM\SYSTEM\CurrentControlSet\Control\FileSystem\LongPathsEnabled` は**この開発機で未設定**
  (= 既定の無効)。「長パスが有効だったから作れた」のではない。
- それでも net9.0 / net10.0 のどちらでも 271 文字のファイル作成・`File.Exists`・`ReadAllText`・
  `File.Move(overwrite: true)` が**すべて成功**する。原因は **.NET ランタイムがパスへ `\\?\` を
  自動付与する**こと(`PathInternal.EnsureExtendedPrefixIfNeeded`)で、**OS 設定ではなく
  ランタイムの性質**である。`\\?\` を明示した 279 文字でも作成・改名が成功。
- したがって「CI で成立しない」懸念は根拠を失う(CI も同じ .NET 9 ランタイム)。

**採った修正**: `MakeLongSettingsPath` の `Assert.InRange(dirNameLength, 8, 200)` の**上限だけ**
250 へ引き上げ(TEMP が短い CI 環境ほど `dirNameLength` が大きくなるため。1 パス構成要素の
限界 255 未満)、fixture を `quarantineLength: 275` にした。**274 未満のあらゆる上限を殺す。**

```
# 修正前(fixture 250)—— M3b 生存
成功!   -失敗:     0、合格:     6、スキップ:     0、合計:     6

# 修正後(fixture 275)—— M3b 撃墜
失敗 kxEdit.App.Tests.SettingsStartupTests.Prepare_quarantines_a_corrupt_file_and_points_at_it_in_full
   Assert.Contains() Failure: Sub-string not found
失敗!   -失敗:     1、合格:     5、スキップ:     0、合計:     6
```

**これは [[net-absence-claims-are-also-verifiable]] に正面から当たる。**「張れない網を張ったと言うのも
嘘だが、張れる網を『張れない』と宣言するのも同じ嘘である」——§10.7 指摘 2 で同じ判断をして
**網が存在した**のに、本タスクではその教訓を自分に適用せず、**環境差の懸念を実測せずに断念の理由に
した**。§10.7 では「差分の定義を API の xmldoc まで戻って読み直したら網は存在した」と書いており、
今回もランタイムの挙動を 1 回確かめれば済んだ。

#### ★ 指摘 2 —— 「最重要」と主張した文言変更に、網が 1 本も無かった

§10.15 (1) は本タスク最大の主張(計画の文言は事実として弱い)なのに、**`RewriteReason` を計画案の
弱い文言へ戻しても App 743 が全 PASS** だった。自分でも実測して確認した:

```
# 修正前 —— V11(RewriteReason を "設定を変更すると、" へ戻す)が生存
成功!   -失敗:     0、合格:   743、スキップ:     0、合計:   743
```

**M3b については「張れなかった」と正直に書いたのに、こちらは網の不在を一言も申告していなかった。**
これが悪い点で、網が無いこと自体より申告の非対称が問題である —— 主張の強さと網の強さが
逆立ちしていた。しかも網は書けないどころか**同ファイルの既存様式そのもの**(文言 assertion は
既に多数ある)だった。

**採った修正**: ヘルパ `AssertNamesTheRealRewriteTrigger` を足し、`Corrupt`(退避失敗)と
`Unreadable` の 2 本から呼ぶ。観測点は 2 つ ——「終了するとき」が載ること(実際の書込契機を
述べている)と、「設定を変更すると」が**載らない**こと(弱い主張を併記する退行も落とす)。
...
失敗 …SettingsStartupTests.Prepare_does_not_point_at_a_quarantine_that_was_never_created
   Assert.Contains() Failure: Sub-string not found
     at …SettingsStartupTests.AssertNamesTheRealRewriteTrigger(String warning) …:line 70
失敗!   -失敗:     2、合格:     4、スキップ:     0、合計:     6
```

`Corrupt`(退避成功)側では呼ばない —— 原本を退避し終えているので上書きの警告自体が不要で、
この文言には `RewriteReason` が入らないため。

#### 指摘 3 —— 自分が見つけた罠が `Missing` 側にそのまま残っていた

`Missing` テストの「退避が走らない(= `.bad` が生えない)ことも押さえる —— 退避は `Corrupt` とだけ
結び付いているという主張の網」は**その主張の網になっていない**。`Missing` は原本が存在しないので、
退避を呼んでも `File.Move` が落ちて `false` を返し、`.bad` は作られない。**観測しているのは
「呼ばないこと」ではなく「呼んでも失敗すること」** —— §10.15 で自分が `FileShare.None` に見つけた
欠陥と**完全に同型**である。同じ穴を、同じファイルの別の場所に残していた。

挙動としては無害(`Missing` で退避が走っても副作用ゼロ)なので**コードは変えず、コメントを
観測できることへ書き直した**: 「`Missing` では退避の有無を観測できない。退避が `Corrupt` に
限られることの網は `Ok` 側(原本のバイト列不変)と `Unreadable` 側(改名可能なロックで固定)が
持つ。ここでは `Prepare` が設定ファイルを作らないことだけを見ている」。

なお `Ok` 側は**本物の網**として効いている(`Assert.Equal(original, File.ReadAllText(path))` が
改名を殺す)ことをレビュアーが確認済み。過大主張だったのは `Missing` のコメントだけである。

#### 指摘 4 —— §10.15 の変異件数が表と食い違っていた

見出し「5 件・1 件生存」と commit `31351ad` 本文「5 件を実測し 4 件を撃墜」に対し、表は 6 行で
5 殺 1 生存。**表が正しい。**見出しを直し、§10.15 に訂正注記を置いた(commit は書き換えない)。
[[rationale-not-just-conclusion]] の型で、**自分が同じ節に書いた表を数え直さずに prose を書いた**
(§10.6 で踏んだ「自分が数えた本数を確かめずに書いた」の再発)。

#### レビュー参考コメント(受容・文言は変えない)

- **`Corrupt`(退避失敗)の「このまま使うと上書きされます」** —— 退避失敗の主因が原本のロック
  だった場合、`AtomicFile` の `File.Replace` も同じ理由で失敗するので上書きされないことがある。
  断定は**安全側**(喪失を警告する向き)で、指示(先にコピー)も無害なので受容する。
- **`Unreadable` の「先に次のファイルをコピーしてください」** —— 読めない原因が持続的なロック /
  ACL ならユーザーも同じ理由でコピーできない。設計が主因と見なす*一時的*ロックなら解除後に
  実行できるので致命ではない。「読み取れるようになってから」相当の一言を足す案はあるが、
  文言をこれ以上長くする判断は本タスクの射程外とし、申し送りに留める。
...
kxEdit.Editor.Tests  成功!  合格:  516 / 合計:  516   (不変)
kxEdit.App.Tests     成功!  合格:  743 / 合計:  743   (不変)
dotnet csharpier check <変更ファイル>  →  EXIT=0
```

テスト本数は変わっていない(既存 3 本へ網を足し、fixture を長くしただけ)。
変異 2 件(M3b / V11)は復帰を確認済み(`src/` の sha256 が commit 状態と一致)。

### 10.17 Task 8 — 前倒し脆弱性レビューの反映(Critical/High/Medium ゼロ・Low 4 件)

CLAUDE.md §3-4 の前倒しレビュー該当(パス操作)として実施。**実装変更は不要**と裁定された。
§10.15 のセキュリティ自己確認は裏付けられ、根拠はむしろ強化された。

#### 追認・強化された点(いずれもレビュアー実測)

- **パス脱出は構造的に不可能。** `".bad"` は区切り文字を 1 文字も含まないため、連結は
  (a) 新しいパス構成要素を作れず (b) 最後の区切り位置を動かせず (c) 末尾要素を `..` にできない
  (`..` + `.bad` = `...bad` は通常のファイル名)。デバイス名・ADS・末尾ドット/空白・`\\?\`・
  ディレクトリをすべて実測し、**危険な位置を指す入力は存在しなかった**。§10.15 の
  「同じディレクトリに落ちる」より強い形の結論。
- **リンク経由の破壊は成立しない。** 宛先がハードリンクのとき実体は無傷、ジャンクションのときは
  `File.Move` が失敗してリンク・リンク先・原本のすべてが無傷。シンボリックリンクは検証機の特権不足で
  未実測だが、**仮に辿る挙動でも攻撃者の能力は増えない** —— その場所にリンクを置ける主体は
  同じトークンで宛先を直接壊せる(昇格プロセスが存在しない)。
- **`.bad` に新しい露出面は無い。** 同一ディレクトリ内リネームはセキュリティ記述子を**完全に保存**する
  (継承のみのファイルで SDDL 同一・明示 Deny ACE も保存)。`.bad` の ACL は `settings.json` と
  **バイト単位で同一**。§3.3(復旧の `File.Move` が ACL を広げうる)とは違い、**ここでは ACL は動かない**。
- **`Unreadable` を退避しない向きは正しい。** 逆にすると「中身が無傷な設定ファイルが `.bad` へ改名され、
  次の終了時に既定値で確定する」= M-11 が直しに来た無音リセットを**より強い形で新設**する。
- **攻撃者が破損を用意できても能力の弱化にしかならない。** 同ユーザー権限が前提で、その主体は
  破損させる代わりに任意の設定を書ける。既定値も安全側へ倒れる
  (`ConfirmRestoreOnStartup=true` / `RestoreOpenFilesOnStartup=false` / `BackupEnabled=true`)。

#### 指摘 1(L-4・修正済み) —— 構造化案を退けた根拠 2 つがどちらも成立していなかった

結論(構造化しない)は維持でよいと判定されたが、**理由節が事実と違っていた**。
詳細と差し替え後の記述は §10.15 の該当節の訂正注記を参照。要点は
①`SettingsLoadStatus` は Core の型なので「App 層の判定を Core へ持ち込む」は偽
②「no-op と失敗が区別できない」は戻り値を `bool` に固定した場合だけの話で、3 値 enum にすれば区別できる。
**xmldoc(`SettingsStore.TryQuarantineCorrupt`)と設計 §10.15 の 2 か所を、成立する記述へ差し替えた。**

#### 指摘 2(L-3・記録) —— TOCTOU / 多重起動で、存在しない原本パスを案内しうる

kxEdit を 2 つ同時に起動すると、両方が破損 `settings.json` を読み、両方が `File.Move` を呼ぶ。
先着が成功し、**後着は `FileNotFound` で `false`** → 「壊れたファイルは退避できませんでした。……
先に次のファイルをコピーしてください: `<原本パス>`」を出すが、**その原本はもう存在しない**
(実体は先着が作った `.bad` にある)。

**§10.6 (c) が「実在しない場所を案内しない」として潰した欠陥と同型**のものが、別ルートから出る。
セキュリティではなく正確性の問題で、実害は誤誘導止まり。**本タスクでは直さない**
(競合の解消は `Prepare` の呼出タイミングの問題で、通知の配線を持つ Task 9 の方が扱いやすい)。
**Task 9 への申し送りとする**(下記)。

#### 指摘 3(L-2・記録・受容) —— 2 回目の破損が、本物の設定に最も近いコピーを黙って壊す

1 回目の破損で作られる `.bad` は**ユーザーの元設定に最も近い唯一のコピー**である。救出前に 2 回目の
破損が起きると `overwrite: true` が**警告なしで**上書きする。§5.4「最新の破損コピーだけを残す」の
直接の帰結で、**受容が妥当**(世代を増やすと `%APPDATA%` に無限に溜まり、掃除する主体が居ない ——
§9 が `.bad` を掃除しないと決めている以上、増やす方が悪い)。緩和するなら「既存 `.bad` があるときは
文言でそれを伝える」程度だが **B4 の射程外**。

#### 指摘 4(L-1・記録・対応不要) —— `.bad` の ACL 前提は機械依存

検証機の `%APPDATA%\kxEdit` には `Liano\CodexSandboxUsers | ReadAndExecute | inherited=True` という
追加の継承 ACE があり、当該グループのメンバーは中身を読める。つまり「`%APPDATA%` は常にユーザー専用」は
この機では成り立たない。ただし **`.bad` の ACL は `settings.json` と同一(実測)**なので、`.bad` が
増やしているのは**アクセスできる主体ではなく、同じデータが残り続ける時間だけ**である。
恒久残置は §9 の明示的判断で、退避の目的(後から見る)と直接トレードオフの関係にある。

#### Task 9 への申し送り

1. **多重起動の後着が、存在しない原本パスを案内する**(上記 指摘 2)。通知を出す直前に
   `File.Exists` で弁別するか、退避失敗側の文言からパスを落とすかは Task 9 で決める。
   §10.6 (c) の `File.Exists` 一本での弁別が前例。
2. **警告文言のパスは切り詰めない**判断のため、`MessageBox` に載る文字列長は原理的に無制限
   (実際には `DefaultPath` 由来なので短い)。**ログ・クリップボード・例外メッセージへ流さないこと**
   (現状の `Prepare` は流していない)。
3. **`SanitizeForDisplay.OneLine` を外さないこと。** `Environment.GetFolderPath(ApplicationData)` は
   既知フォルダーのリダイレクト(HKCU)で変わりうるため、パスに BiDi / 制御文字が混じる可能性は
   理屈上ある。無害化を通す現状の形はこの点で正しい。
4. **`Unreadable` のまま終了すると案内した当のファイルが上書きされる**(§10.15 の申し送り)は
   B5(M-22)側の検討事項として残っている。
...
### 10.20 Task 10 — L5 チェックリスト。「修正前でも全行 PASS」でないことをどう検算したか

`docs/plans/2026-09-02-save-last-line-of-defense-l5-checklist.md` を起こした。**src は 1 行も変えていない。**
設計 §7 の 5 項目に、§10.18 の L5 申し送り 7 点と L-3 の枝を統合して **8 項目**になった。

| # | 項目 | 由来 |
|---|------|------|
| 1 | `Corrupt`(退避成功)の通知 —— タイトル / 語順 / パス / フォーカス / 閉じた後の戻り先 | §7-1 + §10.18 申し送り 1〜5・7 |
| 2 | `Corrupt`(退避失敗・原本あり)—— 文言が「上書きされる」側へ変わる | §7-2 + §10.18 申し送り 7 |
| 3 | `Unreadable`(ロック)—— 文言と、**退避が起きないこと** | §7-3 + §10.18 申し送り 7 |
| 4 | **L-3 の枝**(退避失敗 + 原本消失。パスを 1 つも載せない文言) | §10.18 申し送り 7 の後半 |
| 5 | 陳腐化警告と両方出るときの**順序** | §10.18 申し送り 6 |
| 6 | 初回起動(`Missing`)で警告が出ないこと(誤警告の退行確認) | 新規(§5.2 の「初回起動が該当」の実機側) |
| 7 | 設定が再起動後も維持されること(原子化の回帰) | §7-4 |
| 8 | 大きい文書の Ctrl+S(fsync の副作用) | §7-5 + §10.8 の L5 申し送り |

#### 検算 —— 各項目の「修正前ならどう見えるか」を、推測ではなく実物で確定した

**共通の土台**: `git show main:src/kxEdit.Core/Settings/SettingsStore.cs` を読み、修正前の `Load` が
`try { … } catch { return new AppSettings(); }` の catch-all 1 本で、`Save` が `File.WriteAllText` 直書き
であることを確認した。加えて `git grep -c "設定を読み込めませんでした" main -- src` /
`git grep -c "TryQuarantineCorrupt\|SettingsLoadStatus" main -- src` が**いずれも 0 件**
(= 修正前にはダイアログも退避も状態区別も存在しない)。

| # | 修正前の見え方 | 観測とどう食い違うか | 確定に使ったもの |
|---|---|---|---|
| 1 | ダイアログ 0 枚・`.bad` 生えない・**原本はその場に残る**(そして終了時に既定値で上書き) | ダイアログが出る / `.bad` に壊した内容が入る / **起動中は `settings.json` が存在しない** の 3 か所で食い違う | main の `Load`・`TryQuarantineCorrupt` の `File.Move` |
| 2 | ダイアログ 0 枚 | 出る。加えて**「終了するとき」が読まれ「設定を変更すると」が読まれない** —— これは §10.16 指摘 2 で「網が無い間 App 743 全 PASS のまま生存した」変異(V11)の実機側 | main の `Load` / `SettingsStartup.RewriteReason` / §10.16 の V11 実測 |
| 3 | ダイアログ 0 枚(「読めない」も catch-all で潰れる) | 出る。**さらに `.bad` が生えないことが観測になるのは、ロックを `FileShare.Delete` にしたときだけ** | main の `Load` / §10.15 の fixture 欠陥(`FileShare.None` では「呼んでも失敗する」しか見えない) |
| 4 | ダイアログ 0 枚。**Task 8 時点の実装なら「存在しない原本パスを案内する」** | 「壊れた内容の在りかは分かりません」= パスを 1 つも載せない | `SettingsStartup.Prepare` の `if (!File.Exists(path))` 枝 / §10.17 指摘 2 / §10.18 |
| 5 | 警告は**陳腐化 1 枚だけ** | 2 枚出て、**設定警告が先**に読まれる | main に設定警告が無いこと / `MainForm.OnShown` の順序と §10.18 の M-A / M-B |
| 6 | 出ない(**修正後も出ない**) | **弁別しない**と明記し、「項目 1 の直後に同じビルドで実施する」(出た → 出ない の対で読む)と手順に書いた | `SettingsStartup` の `case Ok: case Missing:` |
| 7 | 設定は**修正前も維持される**。ただし `File.WriteAllText` は **tmp を 1 つも作らない** | 維持(観測 A)は退行確認と明記し、**弁別は tmp の出現(観測 B)**に置いた | main の `Save` / `AtomicFile.Write` の `Path.GetRandomFileName()` 連結 |
| 8 | **実機では A/B できない** | 弁別項目ではないと明記し、§10.10 の実測値を判断材料として併記 | §10.10 の測り直し表 |

**弁別できない観測を「できる」と書かない**ため、§0.3 に 3 件を表で明示した。とくに
**「`%APPDATA%` 配下のパスが 1 行で読まれる」は実パスに対して恒真である** ——
`OneLine(path) == path`(畳み込む対象が無い)なので、`OneLine` の網としては空振りする。
これは §10.19 指摘 B が L3 の fixture で潰した罠と同型で、**同じ罠が手順書側に再発しないよう**
「読まれ方の観測であって `OneLine` の網ではない」と本文へ書いた。

#### 手順を書くにあたって実測したこと(推測で書かないため)

| 実測 | 結果 | なぜ要ったか |
|---|---|---|
| `FileShare.Delete` のロック中の読み書き | `ReadAllText` は `IOException`、`File.Move` は**成功** | 項目 3 の「`.bad` が生えない」が観測になる条件。`FileShare.None` だと「呼んでも失敗する」に化ける |
| Windows PowerShell 5.1 の `[IO.File]::Move` | **3 引数(overwrite)版が無い**(`MethodException`) | 手順のコマンドがその場で落ちないため。アプリ(.NET 9)は 3 引数版を使うので、**同じ API に見えて別物**である旨も本文に書いた |
| `FileSystemWatcher` で短命な tmp を捕まえられるか | 捕まえられる(消えた後でも `Created` が返る)。フィルタ `settings.json.*.tmp` は `session-state.json.*.tmp` を**拾わない** | 項目 7 の唯一の弁別手段 |
| 10 MB の fixture 生成 | `('x'*80 + CRLF) * 130000` = 10,660,000 バイト / 41 ms | 項目 8 |
| 設定書換の契機の行番号 | `MainForm.cs:665`(`OnFormClosing` → `SaveSettingsSafe`)/ `FileController.cs:1571-1575`(`RegisterRecent`) | §10.15 が引いていた `MainForm.cs:594` は **Task 9 の +73 行で陳腐化**していた。手順書には現行値を書いた |
| 設定ダイアログの導線 | **オプション(&O) → 設定(&P)...**・バックアップ間隔の最小値は **5** | PR #42 の L5 チェックリストは「[ツール]>[設定]」と書いており、現行メニューと違う |

#### L5 項目にしなかったものと理由(チェックリスト §6 と同じ)

1. **M-12 の復旧経路** —— `File.Replace` の部分失敗を実機で決定的に起こせない(設計 §7 / §6.1)。
   起こせない以上「起こらなかった」と「直っている」を区別できず、**何を観測しても修正前と同じ結果**になる。
2. **M-12 の文言**(退避先の案内)—— 同じ理由(§10.6 の「L5」節が既に同じ判断をしている)。
   **SR 前提の文言設計はしたが実発声は未確認**、という状態のまま残る。
...
kxEdit は**設定パスを引数で受け取らない**(`Program.Main` は `SettingsStore.DefaultPath` 固定)ので、
**実 `%APPDATA%\kxEdit\settings.json` を壊す以外に破損起動を作る手段が無い**。したがって
チェックリストは次の形にした:

- 冒頭に「この検証は実ファイルを壊す」の警告ブロック。
- **§2.1 で `%APPDATA%\kxEdit\` をまるごと退避**し、`settings.json` の SHA-256 を控えさせる。
  退避先が既にあれば `throw`(= 前回の L5 が復元されていない事故を検出する)。
- **§5 で復元し、SHA-256 の一致まで確認**するまで完了にしない。
- §1.0 で「起動 1 回ごとに settings.json は書き直される」を明示し、**項目ごとに fixture を植え直す**
  ことを要求した(CSV F2 の §1.2 と同型。使い回すと `Ok` 判定になり、その項目は何も検証しない)。

#### 様式

既存の `docs/plans/*-l5-checklist.md` のうち
`2026-09-01-csv-f2-commit-staleness-l5-checklist.md`(直近作・Critical-1 の教訓が入っている)を主に、
`2026-08-31-network-cloud-path-freeze-l5-checklist.md`(§0 の「自動テストで届かない範囲」表)と
`2026-08-22-backup-savepoint-sync-l5-checklist.md`(陳腐化バックアップの作り方)を参照して揃えた。
CRLF・BOM なし・見出し構成(§0 なぜ L5 が必要か → §1 ノウハウ → §2 事前準備 → §3 チェック項目 →
§4 判定 → §5 後片付け → §6 対象外 → §7 束ね直し → §8 実施記録)も既存に合わせている。

#### 検証

```
変更は docs/plans の 2 ファイルだけ(チェックリスト新規 + 本節の追記)。**src 変更 0**
行末: CRLF=574 / LoneLF=0 / BOM なし(既存 *-l5-checklist.md と同じ)
ローカルパス: 実ユーザーホームの絶対パスの混入なし(手順は %APPDATA% / %USERPROFILE% / $env: で書いた)
