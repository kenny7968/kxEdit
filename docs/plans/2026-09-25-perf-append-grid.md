...
- **`--typing`**: 0.52 → 0.71 µs/insert(+37%)。包み直しのたびにブロックを頭から走査するのと、ピースが増えるため。ゲート(5 µs)には余裕がある。§0.2 の「差分構築はしない(YAGNI)」は維持する。
...
- **harness M-3**: 位置 10KB 以上の中央値の平均は 17.79 → 14.71 ms。ただし 1 回ごとの揺れが大きく、多くの位置で変更前後の範囲が重なる(変更後の最大値が変更前の最小値を下回ったのは、10,640・31,920・42,560・53,200・74,480・85,120 の 6 点。21,280・63,840・95,760 の 3 点は重なる)。kxEdit の外の費用(IME/TSF・NVDA)が乗るため。§3.2 の判断基準は Smoke(再現性の数値)で満たしており、harness は改善の向きが一致することの確認とする。
