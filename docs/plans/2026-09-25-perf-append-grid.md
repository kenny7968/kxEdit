...
- 設計書 §9 の末尾へ実施記録を追記する。当初は「マージ後に次のフェーズのブランチに同梱」の予定だったが、ユーザーの指示で本 PR に同梱した(§9.5)。
...
**fixup 66d4639 の再レビュー**(コード品質パスのエージェント): ✅。M-1・I-1・M-2・脆弱性 Minor-2 の反映と、実施記録(ae3bda8)の記述を確かめた。スレッド安全の根拠 (a)(b) のコメントも正確(SplitStats の先読みは呼び出し条件からピースの内側に収まるので、(b) は安全側の記述)。新しい指摘なし。

**品質ゲート**(ae3bda8 時点)
- `tools/pre-merge-check.ps1`: EXIT 0(Editor 639・App 1011 件ほか)。
- `tools/sr-regression.ps1`: 全通過(EXIT 0)。

### Task 7: L5(2026-09-25・自前のスクリプトによる自動確認・publish 8eeca2d)

**環境**: NVDA 起動中(pid 6372・High 整合性。スピーチビューアーは WM_GETTEXT で読めないので、画面から実解像度で切り出して読んだ)。kxEdit はスピーチビューアーと重ならない位置に置いた。publish 8eeca2d 以後の commit は、コメント・テスト・docs と `gridBytes` の明示(値は同じ)だけ。

**方法**(スクリプトは scratchpad に置き捨て)
- `%APPDATA%\kxEdit` をコピーで退避し、新規文書に本文 S(1,419 行・44,364 字・UTF-8 で 82,010 バイト・CRLF。日本語・絵文字・ASCII の単語を混ぜる)を入れた。
- 入れ方は、貼り付け 6 回(3.6〜24KB)と、1 文字ずつの打鍵 5 区間(計 1,221 単位)の交互。打鍵区間は格子点 8,192・20,480・40,960、ブロック境界 65,536、第 2 ブロックの最初の格子点 69,632 をまたぐように置いた。
- 各区間の後と最後に、UIA の DocumentRange の全文が S と一致することを確かめた(11 区間とも一致)。

**UIA の照合**(格子点 4096·k(k=1〜20)・ブロック境界の ±3 文字と、ランダムな 30 か所。計 170 か所)
- 位置(n 文字移動した先の UTF-16 オフセット)・Character・Word・Line のすべてで 170/170 一致。
- Word は、返る文字列が S の該当箇所の部分文字列と一致し、両端が文字種の変わり目で切れていることで判定した(空白・改行の上では直前の単語を返す既存の規則を含む)。

**NVDA の発声**(格子点 8,192(打鍵区間)・40,960・ブロック境界 65,536・第 2 ブロックの格子点 69,632 の 4 か所で、↓↓↑・→×4・←×2・Ctrl+→×2・Ctrl+←×2)
- 行・文字・単語の読み上げは、すべて S の該当箇所と一致した。例: ブロック境界の行で「L1133 はひふへほ x9 たちつてと ブロック kappa かきくけこ」、文字単位で「ク／スペース／ケー／エー」、単語単位で「kappa／かきくけこ」。
- ハーネス側の問題が 1 回あった。Ctrl と → を 1 回の SendInput でまとめて送り、Ctrl が落ちた。Ctrl を別に送る形に直してやり直した。

**後始末**: kxEdit を終了し、作業中にできた hot exit のバックアップを消して、`%APPDATA%\kxEdit` を退避から戻した。settings.json の SHA256 は開始前と一致した。クリップボードは空にした。

**自動では確かめていないこと**: 音声合成の実際の音(ビューアーに記録された文字列までを確認した)、Ctrl+G の行移動を使ったときの発声(キャレットは UIA の Select で置いた)。
- 上の 2 つの実機確認は、**ユーザーの判断で省いた**(2026-09-25)。

### Task 8: PR(2026-09-25)

- PR #90 を作成した。ユーザーの指示で、設計書 §9.5 の実施記録も本 PR に同梱した(マージ後の追記は不要)。
