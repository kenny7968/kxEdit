...
> オフセット昇順 = **IL 上の出現順**)。あわせて「`Main` が `MainForm` 以外の `Form` を
> 先に作る形」も捕まえる一般化ガード(`Form` 派生 ctor より前であること)を置いた ——
> 今日は該当 ctor が `Main` の IL に無いので空虚だが、`new Form()` を挿す変異で
> 実際に武装することを実測で確認してある。`Contains` / 位置比較は走査の偽陽性で
> 緑になりうるので、次の 4 変異を当てて実際に赤くなることを確認した:
> 「合成点の後ろへ移す」「丸ごと削除する」「テスト seam の `(Type?)` 版へ差し替える」
> 「呼び出しの直前に `new Form().Dispose();` を挿す」。
>
> ただし IL 走査で**固定できないもの**は残る:
>
> - **渡した引数の値**(呼出集合では原理的に観測できない)。なお「どの**オーバーロード**を
>   呼んだか」は引数の個数で固定できるので、こちらは守る側にある(値と入口は別の主張)。
> - **実行順**。固定できるのは IL 上の出現順までで、呼び出しを条件分岐に入れる変異
>   (`Environment.TickCount < 0 ? … : default`)は IL 上に現れるため**生存する**(実測)。
> - **実際に推測抑止が効いたかどうか**。`ImeStartupTests`(テーブルが塗られたことの観測)と
>   **L5(実発声)** の担当である。
>
