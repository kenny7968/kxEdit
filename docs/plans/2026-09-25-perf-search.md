...
### Task 7: 変更後の計測(2026-09-25・src は c381f0b と同一・NVDA 起動中)

- **Core.Bench `--search`**(各行の中央値の 3 回の最小 / 中央 / 最大・ms。括弧内は変更前の中央)

| 行 | 最小 | 中央 | 最大 | 変更前 |
|---|---|---|---|---|
| B1 全文化 | 0.805 | 0.816 | 0.848 | 2.030 |
| B2 打鍵 1 回(searcher ごとに全文化) | 1.999 | 2.010 | 2.014 | 3.223 |
| B2s 打鍵 1 回(全文キャッシュを共有) | 0.935 | 1.076 | 1.176 | —(新設) |
| B3 F3 1 回(FindNext + Locate) | 0.000 | 0.000 | 0.000 | 1.803 |
| B3' F3 初回(searcher を作り直す) | 4.076 | 4.332 | 4.651 | 11.871 |
| B4 Shift+F3 1 回(FindPrev + Locate) | 0.000 | 0.000 | 0.000 | 3.628 |

  - B1: P-13 で約 1/2.5。変更前の揺れ(2.030〜2.046)を大きく超える。
  - B2 → B2s: P-13 で 3.22 → 2.01、P-5(a) で → 1.08 ms。打鍵 1 回の件数更新は約 1/3 になった。
  - B3・B4: 表の二分探索になり、1 µs 未満(Stopwatch の表示桁では 0.000)。
  - B3': 悪化せず、11.9 → 4.3 ms に下がった。表の構築(Matches の全列挙と配列)は旧 `Locate` の全列挙と同程度の費用で、差は主に全文化(P-13)による。

- **harness M-5**(publish `pub-after`・CPU ms/実打鍵・3 回の最小 / 中央 / 最大。括弧内は変更前の中央)

| 文書 | 最小 | 中央 | 最大 | 変更前 |
|---|---|---|---|---|
| empty | 10.88 | 12.00 | 15.07 | 10.32 |
| ja10k | 11.16 | 12.28 | 14.23 | 12.00 |
| ja30k | 12.00 | 13.39 | 13.67 | 13.95 |

  CSV は `results-20260925-220221.csv`・`-220318.csv`・`-220414.csv`(NVDA 起動中・`status=completed`)。
  - **M-5 は変更前の揺れの範囲に収まった**(設計書 §3.2 の判断基準では「揺れの範囲」)。理由は次のとおり。
    - 期待される差は、ja30k で 1 打鍵あたり約 2 ms(B2 3.2 ms → B2s 1.1 ms。間引きで数える回数も減る)。一方、M-5 の値の大半は TextBox・IME・NVDA など kxEdit の外の費用(空の文書で約 10〜12 ms)である。
    - CPU 時間の分解能(約 15.6 ms を 14 打鍵で割るので約 1.1 ms 刻み)と 1 回ごとの揺れ(約 ±2 ms)が、期待される差と同程度である。
  - 効果の判定は Core.Bench で行う。採否は PR でユーザーに判断してもらう(§3.2)。

### Task 8: 最終ブランチレビュー(2 パス)と品質ゲート(修正 570c664)

2 つのパスを別々のエージェントで並走させた。各エージェントには scratchpad の専用ディレクトリを割り当て、リポジトリでのビルドを禁止した。

- **脆弱性パス**: マージ可。Critical はなし。
  - **I-1 ① 修正**: 一致位置表の構築時のメモリーのピークが、上限 100 万件で約 220MB だった(実測)。`MatchCollection` が `Match` を全件キャッシュするため。設計書・計画の「配列の肥大化を防ぐ(約 8MB)」とは約 27 倍ずれていた。構築を `Regex.EnumerateMatches` に替えた(割り当ては 0MB。リスト 2 本で十数 MB)。
    - **計画 §0.2 からの逸脱**: 計画は「`Matches` で構築する(同値性の網を増やさないため)」としていた。同値性の網として、`CollectMatches_yields_same_sequence_as_Matches`(病的パターン・ゼロ幅を含み、`Regex.Matches` の列と順序込みで照合する)を足した。既存の `Strategy_matches_old_implementation_for_random_texts`(`Matches` を使う旧経路との照合)も網になる。
    - 旧 `Locate` / `FindPrev` は `Matches` のまま(上限を超えたときの従来経路の OOM リスクは既存の問題。申し送り)。
  - **M-1 ② PR に記載**: 前方に一致がない Shift+F3 の初回は、旧実装では `before` までの走査で終わっていた。新実装では全文を列挙するようになった。ヒットがあれば直後の `Locate` が旧実装でも全件を列挙していたので、合計は同等。
  - **M-2 ① 修正**: 前提が逆向きに崩れた場合は `ArgumentException` が先に出る。`GetText` のコメントに書いた(品質パスの m-6 と同じ指摘)。
  - **M-3 ② 受容**: 表が編集前のスナップショットを次の照合まで掴む。文書化済みで、寿命は searcher の内側に収まる。
- **コード品質パス**: 承認(Critical・Important なし)。
  - **m-1 ① 修正**: `SnapshotSearcher` の doc に古い記述が 3 か所あった。
  - **m-2 ① 修正**: 旧称「材質化キャッシュ」を「全文キャッシュ(と一致位置表)」にそろえた。
  - **m-3 ① 修正**: 説明文の重複を解消した。
  - **m-4 ① 修正**: `ManualDebounceScheduler` を `FakeDebounceScheduler` に改名した(フォルダーの慣例に合わせる)。
  - **m-5 ② PR に記載**: `WinFormsDebounceSchedulerTests` は、遅延を最後の予約から数え直すことを検証していない。時間に依存するテストは flaky になるので足さない。
  - **m-6 ① 修正**: M-2 と同じ。
  - **ミューテーションのスポットチェック**(Task 6 と重ならない 9 変異): すべて殺された。
    - 対象: `DecodeInto` の中間サロゲートの 2 分岐・`WriteRange` の宛先のずれと数え漏れ・`_textCache` の生成と破棄・`Find` と `Dismissed` の取り消し位置。
    - 右部分木の宛先のずれ(MD)を殺したのは、新設の 2 件だけだった(削除しないこと)。
- **再レビュー**(570c664): 承認。レビュアーが、後方参照・balancing group・条件式を含む 10 パターン × 3 通りのオプション、計 60,000 ケースで `Matches` と `EnumerateMatches` の列の差が 0 であることを確かめた。
  - 任意の Nit(同値性のテストに後方参照のパターンを 1 つ足す)は ③ 却下した。裏取りで差がなく、既存の 2 本で網は足りるため。
- **品質ゲート**: `tools/pre-merge-check.ps1` → **EXIT 0**(Core 1592・Editor 639・App 1030、Release と Debug の両方)。
  - 1 回目は、改名したファイルの作業コピーだけが LF で、CSharpier の検査で落ちた。commit の中身の問題ではない。作業コピーを checkout し直して通った。
- **570c664 の後の Core.Bench `--search`**(NVDA 起動中・3 回の中央値の中央): B1 0.836・B2 2.017・B2s 1.176・B3 0.000・**B3' 2.542**(Task 7 では 4.332。`EnumerateMatches` で構築が軽くなった)・B4 0.000 ms。

### Task 9: 件数表示の遅延の目視(2026-09-25・publish は 4fb5c04・NVDA 起動中)

自前のスクリプトで行った(scratchpad に置き捨て)。ステータスの Label を `WM_GETTEXT` で約 5 ms ごとに読んで、変化するまでの時間を測った。画面は PrintWindow で実解像度に撮った。

- **準備**
  - 新規タブに「名前はまだ無い abc ABC」を 2,000 行、貼り付けで入れた。
  - Ctrl+H で置換モードを開いた(G-2 の自動 Hide を避けるため)。
- **1 打鍵ごと**(打鍵の間隔は 600 ms 以上): 'a' → 4000 件(212 ms 後)、'x' → 見つかりません(209 ms 後)、BS → 4000 件(229 ms 後)。約 200 ms たつまで、表示は前の値のままだった。
- **連打**(20〜30 ms 間隔で 3 打鍵): 表示の変化は 1 回だけで、最後の打鍵の約 228 ms 後だった。BS 3 回でも同じだった。
- **チェックボックス**(大文字と小文字を区別): 即時に更新された(14〜25 ms)。件数も 4000 件 ⇔ 2000 件と正しく変わった。
- **打鍵の直後に Enter**: 'b' の 44 ms 後に Enter を送った。
  - 63 ms で「2000 件中 1 件目」になった。
  - 1.5 秒後もそのままで、「2000 件」には戻らなかった。「ab」の件数も 2000 件なので、取り消されずに残った更新が走っていれば「2000 件」と表示されるはずで、上書きがなかったと判別できる。
- **画像**: 実解像度のダイアログで、ステータスの描画を確かめた(「4000 件」と「2000 件中 1 件目」)。
- **`%APPDATA%\kxEdit` の復元**: コピーで退避した。変わっていたのは、窓を動かしたことによる settings.json の窓の大きさだけだった。それを戻し、ハッシュの一致を確かめた。
- 製品側の異常はなかった。

### Task 10: PR(2026-09-25)

- **ユーザー判断**(2026-09-25)
  - M-5 が揺れの範囲に収まったことについて、Core.Bench の結果をもって採用する。
  - push と PR の作成を了承。
- **PR #91** を作成した。設計書 §10.5 の実施記録も本 PR に同梱した(フェーズ 4 と同じ扱い)。
