...
| 論点 | 決定 |
|---|---|
| 統合単位 | **フェーズごとに PR**。main から `feature/perf-<topic>` を切り、pre-merge-check → PR → マージ。CLAUDE.md §7 の「多フェーズの土台改造はブランチに閉じる」には当たらない。各フェーズは互いに独立した挙動不変の改善で、土台を段階的に作り替えるものではないため |
...
1. 本書 §3 と該当フェーズの節を読む。調査記録の関連箇所(指摘 ID で引く)も読む。
   - 前提: 本書と調査記録を含むブランチ(`feature/general-perf-audit`)が main にマージ済みであること。
...
- **脆弱性レビュー**
  - フェーズ 0: 利用者の `%APPDATA%\kxEdit` の退避・復元、プロセスの起動と終了
  - フェーズ 7: パス操作・ファイル削除
  - フェーズ 8: 外部ファイルのパース
  - フェーズ 12: WebView
- **コード品質レビュー**: 新しい共通の seam や抽象を導入するフェーズ
  - フェーズ 0: 計測基盤
  - フェーズ 3: フレーム入力の集約
  - フェーズ 5: public 型 `SnapshotTextCache` と、差し替え可能な間引きスケジューラ
  - フェーズ 9: クリップ対応の描画

**ミューテーション検証**(CLAUDE.md §4-A、ユーザーのグローバル規約「原則実施しない」)
- **CLAUDE.md の列挙に入るもの**: フェーズ 2 の `ComputeCaretPoint` 短絡(キャレット位置の算出)
- **列挙の外だが、厳密な挙動の保証が要るので提案するもの**
  - フェーズ 4 の格子(文字⇔バイトの対応。カーソルと選択の算出の土台)
  - フェーズ 5 の一致位置キャッシュの二分探索(検索エンジンの中核)
  - これらは**列挙の拡張**にあたるので、実施する前に当該セッションでユーザーの承認を得て、実装計画に記録する。
...
| 2 | Handle の破棄後の `BoundingRectangle` は、最後にキャッシュした値ではなく空矩形を返す |
| 4 | 追記ブロックでは、格子点が増えるたびにピースが 1 つ増える(最大で 4KB ごとに 1 つ。内部の診断値 `PieceCount` だけに現れる) |
...
| 7 | リモートのファイルの更新時刻の取得も期限付きになる。従来は UI スレッドで無期限にブロックしえたが、タイムアウト時は「到達不能として記憶」になる |
| 7 | プレビューの `Dispose` から戻った時点で、作業フォルダーがまだ残っていることがある(背景で削除するため) |
| 8 | リテラル検索のプリフィルタは、タイムアウトしたらプリフィルタなしに戻るので、結果は変わらない。効果が出ない場合があるだけ |
| 11 | 配布物が ReadyToRun になり、publish フォルダーが約 2.3MB 大きくなる(zip での増分は未計測) |
...
| 7 | I/O と設定(`perf-io-settings`) | P-11・P-22・P-12・P-15・P-21(a) | 未計測(リモートで約 200 ms/復帰と推定) | 不要 | 0(計測のみ) |
...
| 11 | 起動・生成(`perf-startup`) | P-7・P-16(前半) | 起動 213 ms | 起動確認 | 0(計測のみ) |
...
- 文書はメモリ上で生成する。行の形式と生成式は調査記録 §9.5 のとおりにする。
...
調査記録 §9.1 と §9.5 の手順を tools/ に固定する。調査で使った作業用のハーネスはリポジトリに残っていないので、§9.5 の仕様から作り直す。BOM 付き UTF-8、pwsh 推奨とする。
...
**利用者データの保全**
- `%APPDATA%\kxEdit` には hot exit のバックアップ(未保存の本文)とセッションの状態が入っている。ハーネスはこれを失わない・取り違えないことを最優先にする(前倒しの脆弱性レビューの対象・§3.4)。
- **開始前に中止する条件**
  - kxEdit が起動中である。
  - `backups` の下に 1 件でもファイルがある。未保存の本文が残っている可能性があるため、利用者に処理を任せる。
  - 退避先に前回の退避が残っている(目印ファイルがある)。前回が異常終了した可能性があるので、復元の手順を表示して中止する。
- **退避はコピーで行う**(移動しない)。
  - 退避先には、目印ファイル(退避の日時と元のパスを記録)を置く。
  - 終了時(try/finally)に元へ戻し、ハッシュが一致したら退避と目印を消す。
  - pwsh の強制終了や OS の異常で finally が走らなかった場合でも、元のフォルダーと退避の両方が残るので、次回の起動で検出できる。
...
- レビュー: コード品質レビューと脆弱性レビューを、どちらも前倒しで行う。前者は以後の全フェーズが依存するため、後者は利用者データの退避・復元とプロセスの起動・終了を扱うため。

### 5.5 実施記録(2026-09-24・PR #85)

- **成果物**
  - Smoke `--perf`(`tests/kxEdit.Editor.Smoke/PerfBench.cs`・S1〜S8)
  - `tools/perf-harness.ps1`(M-1〜M-7・`-SelfTest` / `-Recover`)
  - tools/README §3
  - 実装計画と現状値は `docs/plans/2026-09-24-perf-bench.md`(以下「計画」)の実施記録にある。
- **完了条件**
  - 調査記録 §9 と、桁も大小関係も一致した。F-6 ののこぎり型も再現した。値の出典(計測経路)はそれぞれ次のとおり。
    - 全面再描画(Smoke S7・ja10k): 9.05 ms
    - キャレット移動(harness M-2・ja10k): 17〜19 ms。Smoke S1 では 9.23 ms で、差は IME/TSF など kxEdit の外の費用。
    - UIA の全文の矩形: harness M-7(COM 越し)で 2.7 秒、Smoke S8(直接呼出)で 4.0 秒。
    - Ctrl+Tab(harness M-6): 空 4 枚で 95〜100 ms、全条件で 95〜124 ms。
  - 前倒しのレビュー 2 本と、最終レビューの 2 パスを実施した。
- **本節からの精密化**(詳細は計画の実施記録)
  - harness は**空のプロフィール**(既定設定)で測る。
  - kxEdit はコマンドラインでファイルを開けないので、ファイルは Ctrl+O のダイアログで開く。
  - Smoke の窓は、作業領域の左上に置く。
  - 1 操作あたりの WM_PAINT 回数(`paints_per_op`)を出す。
  - harness は入力の効果を自己チェックし、条件を CSV の `env` 行に記録する。
- **以後のフェーズへの申し送り**
  - 現状値は **NVDA 起動中**に採った。前後比較では NVDA の有無を揃えること。harness は `env` 行に NVDA の有無を記録する。とくにフェーズ 5・6 は、NVDA なしで変更前の値を採り直すこと。M-5 は NVDA 起動中に、調査記録より 3〜7 ms 高かった。
  - Smoke `--perf` は計測中にメッセージを汲まないので、BeginInvoke・タイマー・間引きに回した処理を計上しない。その種の変更(フェーズ 3・5 など)は、perf-harness でも効果を確かめること。
  - harness の値は、1 回ごとの揺れが大きいものがある(kxEdit の外の費用が乗る基準値など)。§3.2 の判断基準どおり、3 回の最小〜最大で判断すること。
...
- WinForms のレイアウトと GetPreferredSize の経路でも、WindowText を読まなくなること。
  - 行番号は dotnet/winforms release/9.0 の `Control.cs` のもの: レイアウトは 8656 行付近、GetPreferredSize は 2183 行付近。
  - kxEdit の EditorControl.cs の行番号ではない。
- MSAA(OBJID_CLIENT → WinForms の AccessibleObject.Name → `Control.Text`)の名前が "" のままであること。

**テスト**
- WndProc に WM_GETTEXT / WM_GETTEXTLENGTH の受信回数を数えるテストフックを追加する(既存の `TestHook_*` の流儀に合わせる)。
- 描画中に 0 回であることを確認する。
  - 画面外のホストには WM_PAINT が届かない。描画させる手段(WM_PRINTCLIENT / `DrawToBitmap` / 画面内の窓で `Update()` など)は、実装計画で決める。
...
- `GdiCharMetrics` に、文字列をキーにした上限付きのメモを足す。
  ...
  - **上限は、キーの合計文字数で決める**(例: 1 インスタンスあたり 256K 文字。1 件は 4,096 文字以下)。溢れたら全消去する。
  - 件数で上限を決めると、最悪で 4,096 文字 × 4,096 件 ≈ 32MB になる。これがタブ数倍になるので、件数では決めない。
- 対象は「先頭が非 ASCII の単一コードポイント」以外で、非 ASCII を 1 つ以上含む run。
- ASCII だけの run と単一コードポイントの経路は変えない。

**メモリと置換頻度の見積り**(実装計画で具体化する)
- 最悪時は 1 インスタンスあたり、約 512KB の文字列にオーバーヘッドを加えた程度。それがタブ数倍になる。
- `PixelMapper.OffsetToPx` の prefix 計測(キャレット X と選択)は、キャレット位置ごとに異なるキーを作る。長い非 ASCII 行でキャレットを動かすと、キーが増え続けて全消去が起きうる。
- 全消去の頻度を Smoke の S1 / S3 で確かめ、頻繁に起きるなら「prefix の計測はメモの対象にしない」を検討する。
...
### 6.6 実施記録(2026-09-25・PR #87)

- **成果物**
  - P-24: `ControlStyles.CacheText`
  - P-17: `ControlStyles.Opaque`
  - P-20: スレッドごとの `BufferedGraphicsContext` を使い回して、自前で二重バッファにする
  - P-2: `GdiCharMetrics` の run の幅メモ
  - Smoke `--paint-snapshot`(描画のピクセル比較)
  - 実装計画・計測値・レビューの経緯は、`docs/plans/2026-09-24-perf-paint-cost.md`(以下「計画」)の実施記録にある。
- **完了条件**
  - **計測**: S7・S1・S3 がすべて、変更前の揺れを超えて改善した。値は NVDA 起動中、3 回の中央値。
    - S7(ja10k)8.88 → 6.86 ms
    - S1(ja10k)9.00 → 6.98 ms
    - S3a(ja10k)10.87 → 7.60 ms
    - en10k は 8〜11% の改善
  - **ピクセル不変**: 変更前後の 16 枚が、すべて画素単位で一致した。
  - **NVDA の簡易確認**: ユーザーが実機で行い、OK だった。
  - **目視**: ユーザーの判断で省いた。次の 2 つは未確認のまま残る。
    - リサイズ中のちらつき
    - 部分的な再描画で画面に古い絵が残る不具合(`--paint-snapshot` では写らない)
  - **レビュー**: 前倒しのコード品質レビュー 2 本(`--paint-snapshot` と描画経路の置換)と、最終レビューの 2 パスを実施した。
- **本節からの精密化・訂正**
  - **§6.2 の「右下の角」**: 右下の角は、高さいっぱいに dock する VScrollBar が覆っている。`g.Clear` だけが覆うのは、`_scrollX` のシフトで生じる右端の隙間である。
  - **§6.3 の実装形**: 旧 `Control.WmPaint` に揃えるため、次の 3 点を加えた。
    - バッファ側にクリップを掛ける。
    - `Allocate` が失敗したら、バッファなしで直接描く。
    - 更新矩形が空なら、何もせずに return する。
  - **§6.3 の共有の単位**: static ではなく `[ThreadStatic]` にした。`BufferedGraphicsContext` はスレッド安全でなく、テストは STA スレッドを並行に立てるため。製品の UI スレッドは 1 本なので、実際には 1 個になる。
  - **§6.3 の採否**: 先に測った結果、−0.76 ms(ja10k)/ −0.82 ms(en10k)で、採用した。
  - **§6.4 の全消去**: S1・S3 の計測中に、全消去は一度も起きなかった。prefix もメモの対象のままにした。
  - **§6.5 の「PrintWindow 画像」**: `PW_RENDERFULLCONTENT` で撮っても、撮影の中で全面が描き直される。そのため、描画処理そのものの不変は確かめられるが、画面に古い絵が残る不具合は写らない。
- **意図的な挙動差**(§3.5 にない、フェーズ 1 で受容したもの)
  - バックバッファの DIB を、プロセスの寿命のあいだ持ち続ける。1920×1080 で約 8MB。
  - DIB の色深度は、最初の描画のときの形式のまま変わらない。
- **以後のフェーズへの申し送り**
  - **フェーズ 3・9**: `--paint-snapshot` に次の 2 つを足してから着手する(計画の実施記録を参照)。
    - 描画を起こさずに画面の絵を撮る方法
    - 状態を遷移させて比べる方法
  - **未確定の競合**: Smoke の窓を破棄するときに、「CreateHandle の実行中は Dispose できない」という例外が 1 回出た。
    - 有力な仮説: NVDA が WinForms 標準のアクセシブルオブジェクトを RPC スレッドから読み、そのときに窓が作り直されている。
    - 製品のフォームやタブを閉じる処理でも、理論上は起きうる。
    - 未確定なので、どのフェーズにも割り当てていない。
...
- したがって陳腐化は、コード上は確実に起きる。実機ではまだ確かめていないので、7.1 のテストで確認する。
- これは `2026-08-22-v0.2-release-bug-audit.md` の M-10(実機未確認として記録)と同じ問題で、本フェーズで回収する。
...
- **置き場所**: `logicalLine == _topLine && segIdx < _topSegment` の判定(`EditorControl.cs:2546-2547`)より**後**に置く。折り返し OFF でも `_topSegment` に古い値が残っている場合の挙動を、従来どおりに保つため。
...
- **計測**
  - S8(UI スレッドから直接呼ぶ): 1 万行の所要時間が、1 行の数倍以内になること。
  - M-7(クライアントから COM 越しに呼ぶ): 1 万行が 1 行と同程度(十数 ms)になること。
- **L5**
...
`OnPaint` が描画に使う入力を、1 つの不変値 `FrameInputs` に集める。
- 対象は FrameBuilder への入力だけでなく、FrameBuilder の外で描画が直接読む状態も含む。
  - `RenderFrame` が読む `_scrollX` と `_font`
  - `g.Clear(BackColor)`
  - `_imeCtrl.Draw(g)` が IImeOverlayHost 経由で読むホストの状態
- 状態の収集は 1 つのメソッド(例: `CaptureFrameInputs()`)に切り出す。`OnPaint` は、このメソッドが返した値**だけ**を使って描く。
- こうすれば、描画に新しい入力を足すと必ず `FrameInputs` に入る。
...
- **古いスナップショットを保持しない**: `_lastPaintedInputs` は本文スナップショットを参照する。描画されないタブで ReplaceSource が起きたとき、旧ピース木を次の描画まで保持しないように、`SetSource` / `ReplaceSource` / `ConvertEols` などの全置換の経路で `_lastPaintedInputs` を null にする。
  - null なら必ず Invalidate するので、安全側に倒れる。
  - `UiaTextHostAdapter.cs:68-70` の `_lastLineSegs` にも、同じ配慮がある。
...
| 強調 ON で行が変わる | ≥1 |
| 選択の変化 | ≥1 |
| スクロールを伴う移動 | ≥1(変更前と同じ回数以下) |
| 選択の解除 | ≥1 |

- スクロールを伴う移動では、スクロールのセッターが無条件に Invalidate する。そのうえで `InvalidateIfFrameChanged` も `_topLine` の違いを見て Invalidate するので、`Invalidated` イベントは 2 回になる(変更前も 2 回)。
- 回数を 1 に揃えたい場合は、「最後に無効化した入力」も記録する設計が必要になる。本フェーズでは揃えない。

**オラクル**(`FrameInputs` の入力漏れを検出するためのもの)
- `FrameInputs` 同士の比較だけでは、「描画が読む状態が `FrameInputs` に入っていない」という最も怖い故障を検出できない。`FrameInputs` が等しければ、純関数の出力は自明に一致するからである。
- そこで、生の状態からフレームを組み直して比べる。
  - 描画のたびに、実際に描いた `Frame` と、RenderFrame / IME の描画が使った値を記録する(テストフック)。
  - `CaptureFrameInputs()` と同じ収集コードで、**生の状態から**フレームを組み直すメソッド(例: `BuildFrameFromLiveState()`)を切り出す。
- ランダムな操作列(移動・選択・編集・スクロール・設定変更・IME)を流す。Invalidate を省いた時点ごとに、`BuildFrameFromLiveState()` の結果が記録したフレームと一致することを確かめる。
- 画面外の HostForm では WM_PAINT が来ないので、描画とその記録は、テストフックで同期的に描画させて行う(手段は実装計画で決める)。
...
**`AppendBuffer`**
- **包み直しの条件**(名目の境界ではなく、スナップ後の位置で判定する)
  - 現在の包みにまだ入っていない次の格子点 p を考える。p は、名目位置(格子幅 4KB の倍数)を継続バイトの先へ前方スナップした位置である(`TextChunk.cs:48-59` と同じ規則)。
  - 書込み後に `p < _pos` が成り立ったら(= p が書込済み範囲の厳密に内側に入ったら)、同じ `_block` を `gridLimit = _pos` の新しい `TextChunk` で包み直し、`_chunk` を差し替える。
  - 名目の境界だけで判定すると、次の 2 つの場合に無駄な包み直しになる(ピースが 1 つ増えるだけで、格子点は増えない)。
    - `_pos` がちょうど 4096 になった場合(`< gridLimit` の規則で、4096 の格子点は置けない)
    - 名目点が多バイト文字の途中に当たり、スナップ後の位置が `_pos` 以上になる場合
- **順序は「書込 → 包み直し → 新しいチャンクでピースを作る」**。逆にすると、32KB 以下の貼り付けピースが古い格子を参照し、長い走査が残る。
  - 現行の `Write`(`AppendBuffer.cs:62-68`)は、`_pos += len` の前にピースを作っている。この順序を入れ替える必要がある。
...
- 多バイト文字を名目 4KB 位置にまたがせる(名目点が 3 バイト文字の途中に当たる場合を含む)。
- `_pos` がちょうど 4096 になったときは、包み直さないこと(ピース数が増えないこと)。
...
### 9.5 実施記録(2026-09-25・PR #90)

- **成果物**
  - `TextChunk` の ctor の `gridLimit`: 格子点を `< gridLimit` にだけ置く。既定値では従来と同一。
  - `AppendBuffer` の包み直し: 次の格子点(名目点を前方スナップした位置)が書込済み範囲の厳密に内側に入るたびに、同じブロックを `gridLimit = _pos` で包み直す。順序は「書込 → 包み直し → ピース」。
  - テスト: `AppendBufferGridTests`(新規)・`TextChunkTests` の追加・追記に偏ったファズ。
  - 実装計画・計測値・レビューの経緯は、`docs/plans/2026-09-25-perf-append-grid.md`(以下「計画」)の実施記録にある。
- **完了条件**
  - **計測**(NVDA 起動中、3 回の中央値)
    - S5: のこぎり型が消えた。書込位置 10KB〜96KB で 7.36〜8.47 ms とほぼ一定。変更前は 8.71 → 17.82 → 8.89 ms。
    - Core.Bench `--largeline` の F-6 節の GetChar: 2.8〜19.2 µs → 0.20〜1.26 µs。残る差は格子セル内のオフセット(最大 4KB)に比例するもので、ブロック内の位置にはよらない。
    - `--typing`: 0.52 → 0.71 µs/insert。ゲート(5 µs)は PASS。
    - harness M-3: 位置 10KB 以上の中央値の平均で 17.8 → 14.7 ms(揺れが大きいので、判定は Smoke で行った)。
  - **ミューテーション検証**(ユーザー承認): 13 変異のうち 10 が殺され、3 は等価変異で生存した。最終レビューで 4 変異の再現と 4 変異の追加を行い、すべて記録どおり・殺された。
  - **L5**: 自前のスクリプトで自動確認した。新規文書に 82KB を貼り付けと打鍵で入れ、格子点とブロック境界の前後で、UIA の照合 170 か所(位置・文字・単語・行)と NVDA の発声 4 か所が、すべて元の文字列と一致した。実際の音声と Ctrl+G での移動の実機確認は、ユーザーの判断で省いた。
  - **レビュー**: 各タスクの仕様レビューと最終レビューの 2 パス。fixup の再レビューも行った。脆弱性パスでは、RPC スレッドの読みと UI スレッドの書込の並行ストレステスト(約 790 万回照合・不一致 0)も行った。
- **本節からの精密化・訂正**
  - **§9.1 の `gridLimit` の既定値**: `int.MaxValue` とし、ctor の中でバイト長に切り詰める。初期と繰上げの包みは `gridBytes: BlockBytes` ではなく `gridLimit: 0` にした(同じく格子は先頭 1 エントリ)。
  - **§9.1 の包み直しの構築**: 前の包みの格子を引き継がず、毎回全体を走査する。費用は無視できることを実験で確かめた(`--typing` の悪化はピースの増加による)。
  - **§9.2 の CR/LF**: 「格子点の直後に LF が後から書かれても補正が効く」は、厳密に内側に置く規則のもとでは起きない。格子点 x を置く時点で s[x] は必ず書込済みで、後から LF が来るのは格子点のないフロンティアだけである。安全性の結論は変わらない。
  - **§9.3 の網**: 「既存の `TextSnapshotGetCharEquivalenceTests`(ゼロ領域の網を含む)」の前提は崩れた。最初の挿入ですぐ包み直すので、探針テストはゼロ領域の焼き付けを検出しなくなった。この網は `AppendBufferGridTests`(格子点の位置の検証と、古いスナップショットの照合)が担う。
  - **§9.2 の「最大 16 ピース」**: 正確には 1 ブロックあたり最大 15(格子点は 4096〜61440 の 15 点)。
- **意図的な挙動差**
  - §3.5 のフェーズ 4 の行のとおり、追記ブロックでは格子点が増えるたびにピースが 1 つ増える。1M 字の打鍵で 16 → 247。
  - §3.5 の「内部の診断値 `PieceCount` だけに現れる」は厳密には不足で、打鍵の処理量にも現れる(`--typing` +37%)。Bench の「7 連続タイピング断片化」も Δ2 → Δ4(ゲート ≤50)。
- **以後への申し送り**(どのフェーズにも割り当てていない)
  - 同じブロックの別の包み同士の隣接マージ(`TextBuffer.Splice` の左マージで、下地の配列が同じでバイトが連続していれば新しい包みで結合する)。ピースの増加と `--typing` の悪化がほぼ解消する(実験で 0.51 µs・16 ピース)。本フェーズは Splice の中核に触れない方針だったので見送った。
  - `WordBoundary.cs` の「約 17 倍・22.5 ms」の再計測。
  - 貼り付けの塊で格子点が多バイト文字の途中に来る形を、元の文字列を正解にして確かめるテスト。
...
- 範囲の端にかかるピース(先頭と末尾)は、`CharToByte` でバイト境界を求めてから直接デコードする。
- 始端がサロゲートの中間の場合: そのコード点を 2 char の一時バッファへデコードして `[1]` だけ書く。
- 終端がサロゲートの中間の場合: そのコード点をデコードして `[0]` だけ書く。
- **書込数が length と一致することを、実行時に検査して、一致しなければ例外を投げる**。
  - `Debug.Assert` では Release で何も検査されず、前提が崩れたときに末尾が `\0` の文字列を黙って返してしまう(従来は長さが違うだけだった)。
  - 検査のコストは小さい。
...
- **構築を始めるのは `Locate` と `FindPrev` だけ**にし、この 2 つを配列で答える(二分探索)。
- **`Count` は、構築済みなら配列の長さを返し、未構築なら従来どおり `Regex.Count`** とする。
  - 検索語の打鍵では searcher が作り直されるので、キャッシュは再利用されない。`Count` が構築(`Matches` の全列挙と配列の確保)を引き起こすと、`Regex.Count`(Match オブジェクトを作らない)より重くなり、M-5 がかえって悪化するため。
- **FindNext は置き換えない**。`Regex.Match(text, from)` は Matches の集合と一致しない(例: "aa" を "aaa" で探すと、Matches は (0,2) だけだが、`Match(text, 1)` は (1,2) を返す)。

**構築時の条件**
- 配列で二分探索してよいのは、Index が**狭義単調増加**の場合だけである。
  - 同じ Index が重なる場合、現行の `Locate` は線形走査で「最後に一致したもの」を返す。この意味論を保つため、狭義単調でなければ配列を線形に走査する形にして、従来と同じ順序・同じ break 規則にする。
...
- 重なり("aa" と "aaa")、単調でない場合(同じ Index の重複を含む)、タイムアウト、上限超え。
- `Count` が未構築のときに配列を構築しないこと。
- F3 と置換で発声する「N 件中 M 件目」の文字列が新旧で一致すること(fake announcer)。
...
- **L5**: 不要とする。理由は次の 2 つ。
  ...
  - P-14 の `Locate` の結果は「N 件中 M 件目」として**発声される**(`SearchController.cs:255-257`, `:471-474`)。ただし、発声する文字列が新旧で一致することを fake announcer のテストで確かめるので、実機の確認は省略できる。
- 件数表示の遅延は目視で確認する。

### 10.5 実施記録(2026-09-25・PR #91)

- **成果物**
  - P-13: `TextSnapshot.GetText` を `string.Create` の 1 コピーにした。端のピースは `TextChunk.DecodeInto` で直接デコードする(`GetSubstring` は削除)。書いた数が長さと合わなければ例外にする。
  - P-5(a): public の `SnapshotTextCache` を新設した。`SearchController` が所有し、照合条件が変わっても使い回す。
  - P-5(b): `IDebounceScheduler` と `WinFormsDebounceScheduler` を新設した。検索語の打鍵だけ、件数表示の更新を 200 ms 間引く。
  - P-14: `MatchPositions`(一致位置の表)を新設した。`Locate` / `FindPrev` を二分探索で答える。
  - 計測: Core.Bench `--search`。
  - 実装計画・計測値・レビューの経緯は `docs/plans/2026-09-25-perf-search.md`(以下「計画」)の実施記録にある。
- **完了条件**
  - **計測**(NVDA 起動中。3 回の中央値の中央)
    - Core.Bench `--search`(ja30k 相当・3MB): 全文化 2.03 → 0.84 ms、検索語の打鍵 1 回 3.22 → 1.18 ms、F3 1.80 ms → 1 µs 未満、Shift+F3 3.63 ms → 1 µs 未満、F3 の初回 11.87 → 2.54 ms。
    - harness M-5 は、変更前の揺れの範囲に収まった(ja30k 13.95 → 13.39 ms)。値の大半は kxEdit の外の費用(TextBox・IME・NVDA)で、CPU 時間の刻み(約 1.1 ms)と揺れ(約 ±2 ms)が、期待される差(約 2 ms)と同程度だった。§3.2 に従い、ユーザーの判断で Core.Bench の結果をもって採用した。
  - **L5**: §10.4 のとおり不要。「N 件中 M 件目」の発声は、特性テスト(旧実装で PASS した後に P-14 を入れた)で不変を確かめた。件数表示の遅延は、実アプリで目視と計時を行った(打鍵から約 210〜230 ms 後に 1 回だけ更新・チェックボックスは即時・直後の Enter で上書きされない)。
  - **ミューテーション検証**(ユーザー承認): P-14 の 10 変異がすべて殺された。最終レビューの 9 変異もすべて殺された。
  - **レビュー**: 各タスクの仕様レビュー、Task 3・4 の前倒しのコード品質レビュー、最終レビューの 2 パスを行った。
- **本節からの精密化・逸脱**(詳細は計画 §0.2 と実施記録)
  - **§10.1**: `GetSubstring` の端点のテストは、後継の `DecodeInto` に対して書いた(`GetSubstring` は削除した)。前提が逆向きに崩れた場合(数えたより多くデコードされる)は、`InvalidOperationException` より先に `ArgumentException` が出る。
  - **§10.2(b) の取り消し点**
    - `UpdateCount` の先頭でも取り消す。
    - 検索の実行では、条件が空の早期 return の**後**で取り消す。
    - **タブを閉じる経路では取り消さない**(逸脱)。取り消すと、最後の打鍵の件数が出ないまま古い件数が残るため。満了時の `UpdateCount` はアクティブ文書を数えるので、閉じた文書を掴み直さない。
  - **§10.3 の「配列の肥大化を防ぐ」**: `Regex.Matches` で列挙すると、`MatchCollection` が Match を全件キャッシュし、上限 100 万件で約 220MB になった(最終レビューの脆弱性パスで実測)。構築は `Regex.EnumerateMatches` で行う。同じ列であることはテストで確かめた。旧経路の `Locate` / `FindPrev` は `Matches` のまま。
  - **§10.3 の補足**: 構築の失敗(上限超え・タイムアウト)もスナップショットごとに記憶し、同じスナップショットでは従来の経路で答える。タイムアウトした最初の 1 回は、構築と従来の経路の 2 回ぶん待つ。
- **意図的な挙動差**
  - §3.5 のとおり: 検索語の打鍵では、件数表示が最後の打鍵から 200 ms 後に更新される。
  - 観測できる差: 前方に一致がない Shift+F3 の初回は、旧実装では前方までの走査で終わっていたが、新実装では全文を列挙する(表の構築)。ヒットがあれば、直後の `Locate` が旧実装でも全件を列挙していたので、合計は同等。
- **以後への申し送り**(どのフェーズにも割り当てていない)
  - 旧経路の `TextSearcher.Locate` / `FindPrev` / `Count` の列挙も `EnumerateMatches` にする。表の上限を超えたときの従来経路は、件数に比例して Match を確保する(`.` × 32M 字で約 7GB。既存の問題)。
  - `WinFormsDebounceSchedulerTests` は、遅延を最後の予約から数え直すことを検証していない(時間に依存するテストは flaky になるので見送った)。
...
- **採らない案: タイトルとステータスの値比較**
  - WinForms の `Control.Text` セッターは、同じ値なら何もせずに return する(dotnet/winforms release/9.0 の `Control.cs` 3285-3287 行: `if (value == Text) return;`)。
  - しかも比較のための `Text` の getter が `GetWindowText` を呼ぶので、kxEdit 側で値を比べても省けるものはない。
  - 調査記録 §9.2 の「タイトルの設定 約 2.5 ms」は、タブ切替でタイトルが実際に変わる場合の値である。
...
- **外部変更チェック**: パスのないタブへ切り替えたとき、`BeginInvoke` を投げないこと。
...
### 11.6 実施記録(2026-09-26・PR #92)

- **成果物**
  - P-6: `BackupCoordinator.DocBackup` に、最後に署名を計算したスナップショットへの弱参照を持たせた。未保存で、参照が同じで、`!ForceWrite` のときは、全文化とハッシュを省く。
  - 外部変更チェック: パスのないタブへの切替では、`BeginInvoke` を投げない。
  - `tools/perf-harness.ps1 -SessionRestore`
  - 調査結果: §11.1.1
  - 実装計画・計測値・レビューの経緯は `docs/plans/2026-09-25-perf-tab-switch.md`(以下「計画」)の実施記録にある。
- **完了条件**
  - **計測**(harness M-6・NVDA 起動中・3 回)
    - 揺れの範囲を超えて改善したのは、セッション復元 ON の「3 ファイル未保存+空」(中央値 110.2 → 105.5 ms)。P-6 が効くのは未保存タブがあるときだけなので、他の条件は揺れの範囲で変わらない。
    - OFF の「3 ファイル+空」は、変更後の最小が変更前の最大を 1.2 ms 上回った。変わるコードはナノ秒級だけなので、揺れと判断した。PR に記載し、採否をユーザーに委ねた。
    - 固定費の大半(UIA・NVDA・カーネル)は、kxEdit では減らせないと結論した(§11.1.1)。
  - **L5**: 不要(フォーカス移動と発声の順序には触れていない)。
  - **レビュー**
    - Task 1(harness)の前倒しの脆弱性レビューを行った。
    - 最終レビューの 2 パス(コード品質・脆弱性)を行った。
    - 指摘は fixup で反映した。
  - **ミューテーション検証**: 行わない(§3.4 の対象外)。陰性対照を 4 種試し、どれもテストが落ちることを確かめた。
- **本節からの精密化・逸脱**(詳細は計画 §0.2)
  - §11.1 の NVDA あり / なし・windows-mcp あり / なしは、切り替えずに ETW の帰属で見積もった。
  - §11.3 は 2 項目とも採らない(ユーザー判断)。そのため §3.5 のフェーズ 6 の行は適用しない。
- **意図的な挙動差**: なし。
- **以後への申し送り**(どのフェーズにも割り当てていない)
  - **既存の欠陥**(最終レビューの脆弱性パスが発見。main でも同じ)
    - 保存してクリーンになった後、Undo で「最後に退避した内容」へ戻すと、その状態が退避されない。hot exit やクラッシュで失われる。
    - 原因: 削除の分岐と `ReconcileMapMaintenance` が `LastSig` を残し、`BackupPlanner.Decide` は未保存のとき `HasBackup` を見ない。
    - 直すときは、P-6 の省略条件にも `HasBackup` を加えること。
  - フォーカス移動の二重化(§11.3)は未確認のまま残る。
...
**現在の例外の扱い**(経路によって異なる)
- プローブの work(`FileReachabilityProbe.cs:183-206`)は、`File.Exists` / `Directory.Exists` の例外をすべて握り、`(false, false)` = 到達不能として返す。呼び出し側はこれを TTL 付きで記憶する。
- UI スレッド側の `File.Exists` + `GetLastWriteTimeUtc`(`FileTimestampProvider.cs:112`)の例外は、null になるだけで記憶しない(`:114-124`)。
- Reachable の定義は「ファイルが在る**または親フォルダーが在る**」である。

**変更**
- 更新時刻取得用の**別メソッド**をプローブに追加する(例: `ProbeTimestampWithTimeout`)。
  - `ProbeSaveTargetWithTimeout` は保存経路と共用しているので、戻り値の型を変えない。
- 新メソッドの work は、次の順で処理する。
  1. `FileInfo` を 1 回作る。
  2. Exists を見る。
  3. ファイルがなければ、従来どおり親の `Directory.Exists` を見て Reachable を決める。
  4. ファイルがあれば、LastWriteTimeUtc を取る。
- 結果は `(Reachable, Exists, LastWriteUtc, Error)` で返す。
- ローカルも `FileInfo` 1 回にする。

**等価性の要点**
- Exists 系(`FileInfo.Exists` と `Directory.Exists`)の例外は、従来どおり「到達不能として記憶」にする。
- 更新時刻取得の例外は、`Error` を返して「null で、記憶しない」にする(従来の UI スレッド側の扱いと同じ)。
- 1601 年のガードを保持する。
- `File.Exists` の意味論(ディレクトリは false)を保持する。`FileInfo.Exists` も同じである。
- 更新時刻の取得も期限の内側に入るので、タイムアウト時は「到達不能として記憶」になる(意図的な挙動差 §3.5)。従来は UI スレッドで無期限にブロックしえた。
...
3. **リテラル検索の全文プリフィルタ**
   ...
   - 全文への `IsMatch` にも 1 秒のタイムアウトがかかる。上限 64MB のファイルで IgnoreCase や非 ASCII を含むと、遅いマシンでは超えうる。
   - **プリフィルタが `RegexMatchTimeoutException` を投げたら握り、プリフィルタなしの行単位照合に戻る**。従来より悪くならない(意図的な挙動差 §3.5 は「効果が出ない場合がある」だけになる)。
...
**フェーズ 3 の前提と `_lastFrame` の扱い**
- 差分は `_lastPaintedInputs` から取る。
- WM_PAINT の更新領域は、保留中の無効領域の和である。そのため、部分的に描いた後に `_lastPaintedInputs` を現在の入力へ更新してよい。画面全体が現在の入力で描いた状態と一致するからである。
- `_lastFrame`(`EditorControl.Paint.cs:79`、テストフックが観測する)は部分フレームになる。本番コードで読む箇所はないが、テストの期待値は「全体フレームを行で絞ったもの」に合わせる。
...
- **無効化**: `ApplyAppearance` の**先頭と末尾の両方**で無効化する。
  - 現行はキャッシュを UI スレッドでしか読まないので、この問題はない。volatile で公開すると、RPC スレッドが `ApplyAppearance` の途中の状態を読みうる。そのため先頭でも無効化する。
  - キーに Metrics を含めるので、これは防御の二重化にあたる。先頭で無効化すれば、「ApplyAppearance より前の時点の答え」として線形化できる。
...
- **成果物**
  - `UiaTextHostAdapter._lastLineSegs` を、不変クラス `LineSegsCache(Snap, Line, Wrap, Metrics, Segs)`
    の volatile 参照に変更した。折り返し ON の行問い合わせ(`LineStartOf` / `LineEnd` /
    `LineEndNoBreakOf`)がキャッシュにヒットすれば、RPC スレッドが UI スレッドへ同期 Invoke せずに
    即答する。
  - `TryFindVisualSegment` の判定順序を「折り返し OFF → Handle のガード → 空行(スナップショットのみで
    判定・Invoke なし) → キャッシュ照合(ヒットなら即答) → 従来どおり Invoke」に変更した。ミス時の
    計算(`LineLayout.Wrap` / `MeasureRun`)は従来どおり UI スレッドだけで行う。
  - `ApplyAppearance` の先頭でもキャッシュを破棄するようにした(末尾の破棄は既存のまま維持。
    キーへの Metrics 照合との防御の二重化)。
  - Invoke 回数を数えるテストフック `EditorControl.TestHook_LineSegsInvokeCount` を追加し、既存の
    Hit/Miss カウンタとあわせて `Interlocked` にした(RPC スレッドからの加算に対応)。
  - Smoke `--perf` に S9(S9a/S9b)を追加した。折り返し ON(40 桁)の ja10k に対し、say all 相当の
    行読み(`LineEnd` → `LineStartOf` → `LineEndNoBreakOf`)をワーカースレッドから行い、1 歩あたりの
    Invoke 回数を `param` 列に出す。S9a は UI スレッドがアイドル、S9b は全面再描画を挟む条件。
  - 実装計画・詳細な計測値・テスト・レビューの経緯・陰性対照の結果は
    `docs/plans/2026-09-26-perf-uia-wrap-cache.md`(以下「計画」)の実施記録にある。
- **完了条件**
  - **計測**(計画 §0.1・S9a/S9b・変更前後各 3 run)
    - S9a: median の中央値が 0.349ms → 0.086ms(変更後が変更前の min–max 下限 0.108ms を下回る)。
      `param`(Invoke/歩)は 3.01 → 0.50。
    - S9b: median の中央値が 19.035ms → 0.505ms(変更後が変更前の min–max 下限 6.098ms を大きく
      下回る)。`param` は 3.00 → 0.51。
    - 判断基準(設計書 §3.2)に照らし、S9a/S9b とも改善を達成。`param` の変化(約 3.0 → 約 0.5)は、
      ja10k の 1 論理行(72 桁)が折り返し 40 桁で 2 視覚行になるため、論理行が変わる歩だけ Invoke
      するという設計どおりの見込み値と一致した。
  - **L5**(計画 §0.4・実施記録 (5)): 折り返し ON(40 桁)で、↓×10・↑×10・say all の発声が
    変更前後で完全一致(2 回実施)。事前から存在する「say all が論理行 2 行ぶんで停止する」挙動は
    変更前後で同一であり、本フェーズの回帰ではない。
  - **レビュー**: Task 1〜3 のタスクレビューはいずれも Critical/Important の指摘なしで承認された。
    最終レビュー(コード品質パス・脆弱性パスの 2 パス・別エージェント)は両パスとも**マージ可**で、
    Critical/Important の指摘はなし。Minor の指摘(掃引テストの前提 assert・Task 1 テストの
    フォント依存の記述・待ち時間・S9 の中央値の読み方)は fix wave で反映した(計画の実施記録 (7))。
  - **ミューテーション検証**: 行わない(計画 §0.3・CLAUDE.md §4-A)。Task 2 の最後に陰性対照 3 件を
    実施し、いずれも狙った対象テストのみが FAIL することを確認した(計画の実施記録 (2))。
- **本節からの精密化・逸脱**(詳細は計画 §0.2・実施記録 (4))
  - キャッシュを LRU にはしなかった(単一エントリのまま)。say all の 1 歩は「今の行の `LineEnd` →
    次の行の `LineStartOf`/`LineEndNoBreakOf`」であり、ミスは常に「まだ求めていない次の論理行」
    でしか起きないため、既に求めた行へ戻る場合にしか効かない LRU は効果が薄いと判断した(YAGNI)。
    S9 の `param`(約 0.5 回/歩)が見込みどおりであることで裏付けた。
  - 空行は RPC スレッドで即答するようにした(設計書には明記のない精密化)。判定は `TextSnapshot`
    だけで済み(スレッド安全)、Handle のガードの後・キャッシュ照合の前で null を返す。
  - Invoke の後にもう一度キャッシュを見るようにした。Invoke を待つ間に別の問い合わせが同じ行を
    埋めていることがあるため。
  - 計測基盤として Smoke `--perf` に S9 を新設した(既存の S1〜S8・harness には RPC スレッドからの
    行読みを測るものがなかったため)。
  - Task 2 のテストフィクスチャは、`EditorControl` の既定コンストラクタのフォント名(半角
    「MS ゴシック」)が実在フォントに解決されず比例フォントへフォールバックする既知の未起票課題により、
    計画が想定した固定の折り返し境界値(`[0,4)[4,8)[8,10)`)と実際の値(`[0,4)[4,9)[9,10)`)が
    食い違った。ctor のフォント修正は本フェーズのスコープ外と裁定し、該当テストは UI スレッドで
    実測した値を期待値とする形に修正した(製品コードの折り返しロジックは変更していない)。
- **意図的な挙動差**: なし(計画 §0.5)。
- **以後への申し送り**
  - say all が論理行 2 行ぶんで停止する事前挙動(変更前後とも同一)の原因調査。NVDA 側の say all
    継続条件か、kxEdit の UIA テキスト範囲(`Move`/`ExpandToEnclosingUnit`)の応答かを切り分けて
    いない。
  - `ApplyAppearance` 先頭でのキャッシュ破棄は、キーへの Metrics 照合との防御の二重化であり、
    単独の効果を検証する専用テストは置いていない。
  - ctor の既定フォント名(半角「MS ゴシック」)が実在フォントに解決されない件は、本フェーズの
    スコープ外の既知課題として別途起票が必要(別件)。
  - **既存のレース(脆弱性パス M-1・受容)**: `OnSnapshotChanged` 直後に、進行中だった Invoke が
    古いスナップショットをキーにしたエントリを書き込みうる(`ReferenceEquals` によるキー照合で
    実害はないが、任意対応として書込み直前に現在スナップショットとの `ReferenceEquals` ガードを
    足す案がある)。
...
**変更**: `release.yml` の publish に `-p:PublishReadyToRun=true` を足す(意図的な挙動差 §3.5: publish フォルダー +2.3MB。zip での増分は未計測なので、本フェーズで測る)。
...
- 比較の相手は、**前回適用した設定値**(要求したフォント名とサイズ)をフィールドに保持したものにする。
- `_font.Name` とは比べない。英語 UI では「MS Gothic」が返り、存在しない名前ではフォールバック名が返るので、毎回作り直しになる(安全側だが効果が出ない)。
...
### 16.4 実施記録(2026-09-26・PR 番号は作成後に記入)

- **成果物**
  - P-16: `EditorControl.ApplyAppearance` は、前回適用したフォントの要求値(`FontRequest(Name, Size)`。
    既定値を補完した後の値)と今回が同じなら、フォント 3 個と `GdiCharMetrics`(幅メモ)を作り直さない。
    初回(ctor の直後)は必ず作り直す。テーマ・表示設定・`_topSegment` のリセット・スクロールバー・
    キャレット・UIA キャッシュの破棄など、フォント以外は従来どおり毎回行う。テスト
    `ApplyAppearanceFontReuseTests`(7 本)。
  - P-7: `release.yml` の publish に `-p:PublishReadyToRun=true` を足した。WebView2 の管理 DLL 3 個
    (`Microsoft.Web.WebView2.Core/WinForms/Wpf.dll`)は `PublishReadyToRunExclude` で対象から外した
    (下の「逸脱」)。README の「リリース」に手順と確認事項を追記した。
  - Smoke `--perf` に S10(同じ設定での `ApplyAppearance` + 全面再描画)を追加した。
  - 実装計画・詳細な計測値・陰性対照・起動確認・レビューの経緯は `docs/plans/2026-09-26-perf-startup.md`
    (以下「計画」)の実施記録にある。
- **完了条件**
  - **計測**(変更前後とも各 3 回・NVDA 起動中。§3.2 の判断基準で、すべて変更前の下限を下回った)
    - 設定変更の適用時間(S10・1 タブぶん): ja10k 10.336 → 7.539 ms、en10k 8.397 → 6.929 ms(P-16)。
    - M-1(起動): 入力受付まで 196.19 → 168.49 ms、窓の表示まで 174.32 → 153.39 ms(P-7)。起動時の
      `ApplyAppearance` は初回=作り直しなので P-16 は効かない。調査記録の −13 ms は NVDA なしの条件なので
      大小を比べない。
  - **起動の確認**: 配布 zip を展開して起動し、Markdown のプレビューを表示できた(WebView2 動作)。
    設定でフォントサイズを変えると本文が変わり、変えずに OK を押しても表示は崩れない(計画の実施記録 (4))。
    WebView2 の DLL を除外した最終の publish でも起動と終了を確認した。
  - **crossgen の警告**: release.yml と同じ publish を `-v:n` で実行し、「0 個の警告」・EXIT 0 を確認した。
  - **レビュー**: タスクレビュー 4 件(Task 4 は実施記録の因果の記述で 1 ラウンドの修正)。最終レビュー
    2 パスはいずれも Critical/Important なし。Minor 3 件を fixup で反映した(計画の実施記録 (5))。
  - **ミューテーション検証**: 行わない(§3.4 の禁止対象)。陰性対照 2 件(使い回しの分岐を外す/
    `_font.Name` と比べる)で、狙ったテストだけが落ちることを確認した。
- **本節からの精密化・逸脱**
  - 比べる値は既定値を補完した後の要求値。スタイルは比べない(`AppSettings` にスタイルの項目がない)。
  - **WebView2 の管理 DLL 3 個を ReadyToRun の対象から外した**(§16.1 にない逸脱)。crossgen2 で再生成すると
    Microsoft の Authenticode 署名が消えるため(最終レビューの脆弱性パス)。プレビューを開くときにしか
    読まないので起動の改善には効かない。
  - 設定変更の適用時間を測る場がなかったので、Smoke `--perf` に S10 を新設した。
- **意図的な挙動差**(§3.5 の行の実測): 配布物が ReadyToRun になり、publish フォルダー(説明書・変更履歴を
  含む)は 3,385,683 → 5,203,283 バイト(+1.82MB)、zip は 1,143,191 → 2,003,728 バイト(+0.86MB)。
- **受容**: 起動後に不足フォントを導入した・DPI/ClearType を変えた場合、同じ設定で OK を押しても幅メモを
  作り直さない。変更前もフォントの変化の通知を処理しておらず、名前かサイズを一度変えれば回復する。
- **以後への申し送り**
  - フォントダイアログで 20pt を選ぶと、設定の表示が「20.3 pt」になる(起動確認での観察。変更前のビルドで
    再現するかは未確認・別件)。
  - ctor の既定フォント名(半角「MS ゴシック」)の件は引き続き別件(§16.2 対象外)。
...
**成果物**
- 本節の末尾に評価結果を追記する。
- 採用する場合は、その案の設計を**新しい日付の設計書** `docs/plans/YYYY-MM-DD-perf-preview-env-design.md` に書き、ユーザーの承認を得てから実装する。
  - 本書に追記しないのは、CLAUDE.md §8 が許すのが「精密化と実施記録の追記」だけで、新しい設計の追記はその範囲を越えるため。
