...
- **完了条件**
  ...
  - **レビュー**: Task 1〜3 のタスクレビューはいずれも Critical/Important の指摘なしで承認された。
    最終レビュー(コード品質パス・脆弱性パス)の結果は本節に追記する。
...
- **以後への申し送り**
  ...
  - `LineSegs_SweepFromWorkerThread_MatchesUncachedUiThreadAnswers` の前提 assert
    (`TestHook_LastLineSegsHitCount > 0`)は、ワーカー開始前にカウンタをリセットしていないため
    実質的に空振りしている(Minor)。
