...
- **完了条件**
  - 調査記録 §9 と、桁も大小関係も一致した。全面再描画 ja10k は 9.05 ms、キャレット移動は 17〜19 ms、UIA の全文の矩形は 2.7〜4.0 秒、Ctrl+Tab は 95〜100 ms。F-6 ののこぎり型も再現した。
...
- **以後のフェーズへの申し送り**
  - 現状値は **NVDA 起動中**に採った。前後比較では NVDA の有無を揃えること。harness は `env` 行に NVDA の有無を記録する。
