...
### 9.8 最終 2 パスの結果(2026-08-30)

CLAUDE.md §3-5 のとおり、コード品質パスと脆弱性パスを**別エージェントで独立に**起動した。

**脆弱性パス**: 本ブランチが新規に導入した堅牢性・安全性の欠陥は**ゼロ**。
探針で潰した範囲を記録しておく(再監査時の省力化のため)。

- 算術: 文書 8 種 × `start` ∈ {`int.MinValue`, -5, -1, 0..len+2, `int.MaxValue`} ×
  `length` ∈ {`int.MinValue`, -5, -1, 0..3, `int.MaxValue`} × 置換文字列 4 種(約 2,600 通り)で
  throw 0 件・戻り値の範囲外 0 件。`Math.Clamp(endLong, s0, CharLength)` は `s0` が先に
  クランプ済みなので `min > max` になりえない(`ArgumentException` 経路なし)。
- 例外安全性: 文字列を**組み終えてから**委譲する順序なので、`GetText` の範囲外や
  `string.Concat` の OOM はバッファに触れる前に飛ぶ。中途半端な状態は残らない。
- リソース保持: GC 探針で、`_lastHit` と `_selectionScope` を両方捕捉した状態から
  `ReplaceSource` → `GC.Collect()` で旧 `TextSnapshot` が**回収される**ことを確認。
  main の `_lastHit` は `MatchSpan?` で参照を持たなかったので、弱参照の追加は正しい選択。
- 世代チェック: `TextBuffer` は編集・**Undo・Redo のすべてで**新しい `TextSnapshot` を作る
  (参照がエイリアスしない)ので「世代が同じなのに内容が違う」は成立しない。
  `ReplacementAt` は 3 戦略すべてで `m.Index == span.Start && m.Length == span.Length` の
  完全一致を要求するため、パターン変更後の古いヒットで余分に消す事故は原理的に起きない。
- 無警告のデータ破壊: 「ReplaceOne 連打 vs ReplaceAll」の差分探針を全文置換
  (文書 7 種 × パターン 10 種)と「選択範囲のみ」(文書 4 種 × パターン 6 種 × 全 `[ss,se)` 組合せ)
  で実施し**差分 0**。スコープ内 ReplaceOne 連打後に選択範囲外の prefix / suffix が
  1 byte も変わらないことも同じ全組合せ(ゼロ幅 `\b` を含む)で**違反 0**。
- `SnapToLogicalCharEnd` 全数(アルファベット {a, CR, LF, high, low} の長さ 4 以下の全文字列 ×
  `[-2, len+2]` の全位置): 出力が常に論理文字境界・`[0, CharLength]` 内・単調・throw なし。
  **§2.2 の「委譲先の再スナップは恒等」はこの全数結果で裏づけられた。**

### 9.9 main 既存バグを 1 件根治した(脆弱性パス High)

`ReplaceAll` の書き戻しが非 Exact な `ReplaceCharRange` だったため、スコープ端が CRLF の
内側にあると、`ReplaceInRange` が組んだ断片と書込先の範囲の長さが食い違う。

実測(**main でも再現**。本ブランチの退行ではない):

| 手順 | 結果 |
|---|---|
| `"a\rXY\nb"` の `[2,6)` を選択 → 「選択範囲のみ」ON | scope `[2,6)` |
| ReplaceOne `XY` → `""` | 本文 `"a\r\nb"` / scope `(2,4)` = CRLF の中間 |
| ReplaceAll `b` → `Q` | **`"a\nQ"`**(期待 `"a\r\nQ"`)= **選択範囲外の CR が黙って削除**。発声は「1 件置換しました」 |

main では `ReplaceOne` が scope を触らないので、編集後の `ReplaceAll` は世代不一致で拒否され
この経路に到達しない。**本ブランチは 2 本目の到達経路を増やしていた。**

書き戻しを `ReplaceCharRangeExact` に差し替えて根治した。両端が境界に乗っている既存ケースでは
prefix / suffix が空になり委譲先が同一なので**挙動不変**。本ブランチがその API を作った当人で
あり、A-14 と同じ「論理文字を巻き込む」欠陥の同型なので、ブランチ内で直す判断をした。

なお `_selectionScope` 再捕捉のコメント「rangeStart は…どちらも文字の途中を指さない」は
**事実として誤り**だったので併せて訂正した(再捕捉の値自体は Exact 置換なら正しい)。

### 9.10 §6 申し送りへの追加(最終 2 パス由来)

- **ゼロ幅ヒットがスコープ外へ挿入されうる。** `WithinScope` は生の UTF-16 span で包含を
  判定するが、`ReplaceCharRangeExact` はゼロ幅のとき挿入点を論理文字の境界まで**後退**させる。
  `scope.Start` が論理文字の内側だと、挿入がスコープの外に落ちる(成功発声つき)。
  実測再現: `"X\rYZ"` の `[2,4)` を捕捉 → `Y`→`\n` で位置 2 を CRLF 内側にする →
  `(?<=\r)` で `Q` を置換 → `"XQ\r\nZ"`。
  正しい直し方は**事後条件で検査する**こと(前置ガードの列挙は原理的に漏れる。
  監査 §9 V-7 の教訓と同型)。`ReplaceCharRangeExact` に「実際に書く範囲」を問う seam が要るので
  次のブランチで扱う。トリガは「スコープ内の置換がスコープ端に CRLF を作る」+
  「ゼロ幅正規表現」の同時成立に限られる。
- **`ReplaceOne` の最悪ケース regex 時間が 2 倍**(分岐 1 と分岐 2 の両方で `ReplacementAt` を
  呼びうるため、1 秒タイムアウトを最大 2 回踏む)。UI が固まる上限が 1 秒 → 2 秒。
  無制限化はしていないので新規の DoS 経路ではない。
- **`ReadOnly` の no-op でもスコープが伸縮し成功発声する。** `ReplaceCharRangeExact` は
  `ReadOnly` 時に何も書かずに戻るが、呼び出し側はそれを見ずに `_selectionScope` を更新する。
  `snap2 == snap` なので世代チェックを通る不正なスコープが残る。**到達性は実質なし**
  (`ReadOnly=true` は CSV モードと保存中の一時解除だけで、前者は `IsCsvModeActive` が弾き、
  後者に `ReplaceOne` が割り込む経路がない)。直すなら `if (ed.ReadOnly) return;` を先頭に。
