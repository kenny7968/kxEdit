...
### A. ミューテーション検証
ミューテーション検証はバグが致命的になりやすい中核機能のみに限定する。

1. 有効（実施すべき箇所）:
    - カーソル移動、テキスト選択範囲の算出、UNDO/REDOの履歴管理アルゴリズム
    - 正規表現による検索・置換エンジンのパース処理や、シンタックスハイライトの字句解析（Lexer/Parser）のコアロジック
2. 禁止（ミューテーションテストを禁止する箇所）:
    - GUIのレイアウト、キーバインドのイベントマッピング、テーマ（配色）の適用処理
    - ファイルの入出力（I/O）処理や、プラグインのロード機構

UIの操作性や見た目に関わる部分、および単純なイベントハンドリングに対するミューテーション検証は、開発生産性の観点で全面禁止。
